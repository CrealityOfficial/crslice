...
namespace cxutil
{
    ...
    Slicer::Slicer(Mesh* i_mesh, const coord_t thickness, const size_t slice_layer_count, 
                   bool use_variable_layer_heights, std::vector<AdaptiveLayer>* adaptive_layers, MeshGroup* meshGroup)
        : mesh(i_mesh)
    {
        const SlicingTolerance slicing_tolerance = mesh->settings->get<SlicingTolerance>("slicing_tolerance");
        const coord_t initial_layer_thickness = meshGroup->settings->get<coord_t>("layer_height_0");
	    ...
	    {
		    std::fstream out;
		    out.open("layer", std::ios::out);
		    if (out.is_open())
		    {
			    for (size_t i = 0; i < layers.size(); ++i)
			    {
				    const SlicerLayer& slayer = layers.at(i);
				    for (size_t j = 0; j < slayer.segments.size(); ++j)
				    {
					    const SlicerSegment& segment = slayer.segments.at(j);
					    out << segment.start << " " << segment.endOtherFaceIdx << std::endl;
					    out << segment.start << " " << segment.end << std::endl;
				    }
				    for (const std::pair<int, int>& p : slayer.face_idx_to_segment_idx)
				    {
	    ...
	    {
		    std::fstream out;
		    out.open("polygon", std::ios::out);
		    if (out.is_open())
		    {
			    for (size_t i = 0; i < layers.size(); ++i)
			    {
				    const SlicerLayer& slayer = layers.at(i);
				    const cura::Polygons& polygons = slayer.polygons;
				    for (size_t j = 0; j < polygons.size(); ++j)
				    {
					    const ClipperLib::Path& path = *polygons[j];
