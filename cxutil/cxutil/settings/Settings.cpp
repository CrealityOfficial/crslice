...
#include "cxutil/math/floatpoint.h" //For FMatrix3x3.
...
namespace cxutil
{
    ...
    Settings::Settings(Settings* _parent)
        : parent(_parent)
        , extruders(nullptr)
    {
    ...
    template<> ExtruderTrain& Settings::get<ExtruderTrain&>(const std::string& key) const
    {
        ...
        return extruders->at(extruder_nr);
    }
    ...
    const std::string Settings::getAllSettingsString() const
    {
        ...
        {
            char buffer[4096];
            snprintf(buffer, 4096, " -s %s=\"%s\"", pair.first.c_str(), cxutil::Escaped{ pair.second.c_str() }.str);
            sstream << buffer;
        }
        return sstream.str();
    }
