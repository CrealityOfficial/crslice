...
#include "cxutil/math/ListPolyIt.h"
namespace cxutil
{
    /*!
...
{
    template <>
    struct hash<cxutil::ProximityPointLink>
    {
        size_t operator()(const cxutil::ProximityPointLink& pp) const
        { // has to be symmetric wrt a and b!
            return std::hash<cxutil::Point>()(pp.a.p()) + std::hash<cxutil::Point>()(pp.b.p());
        }
    };
}//namespace std
