...
namespace cxutil
{
    Polygons PolygonConnector::connect()
    {
        ...
        while (!to_connect.empty())
        {
            ...
            cxutil::optional<PolygonBridge> bridge = getBridge(current, to_connect);
            if (bridge)
            {
    ...
    cxutil::optional<PolygonConnector::PolygonBridge> PolygonConnector::getBridge(ConstPolygonRef from_poly, std::vector<Polygon>& to_polygons)
    {
        ...
        cxutil::optional<PolygonConnector::PolygonConnection> first_connection;
        cxutil::optional<PolygonConnector::PolygonConnection> second_connection;
        ...
        { // We didn't find a connection which can make a bridge
            ...
            cxutil::optional<PolygonConnector::PolygonBridge> uninitialized;
        ...
        { // we couldn't find a connection; maybe the polygon was too small or maybe it is just too far away from other polygons.
            return cxutil::optional<PolygonConnector::PolygonBridge>();
        }
    }

    cxutil::optional<PolygonConnector::PolygonConnection> PolygonConnector::getSecondConnection(PolygonConnection& first)
    {
        bool forward = true;
        cxutil::optional<ClosestPolygonPoint> from_a = PolygonUtils::getNextParallelIntersection(first.from, first.to.p(), line_width, forward);
        if (!from_a)
        { // then there's also not going to be a b
            return cxutil::optional<PolygonConnector::PolygonConnection>();
        }
        cxutil::optional<ClosestPolygonPoint> from_b = PolygonUtils::getNextParallelIntersection(first.from, first.to.p(), line_width, !forward);

        cxutil::optional<ClosestPolygonPoint> to_a = PolygonUtils::getNextParallelIntersection(first.to, first.from.p(), line_width, forward);
        if (!to_a)
        {
            return cxutil::optional<PolygonConnector::PolygonConnection>();
        }
        cxutil::optional<ClosestPolygonPoint> to_b = PolygonUtils::getNextParallelIntersection(first.to, first.from.p(), line_width, !forward);


        const Point shift = turn90CCW(first.from.p() - first.to.p());

        cxutil::optional<PolygonConnection> best;
        coord_t best_total_distance2 = std::numeric_limits<coord_t>::max();
        for (unsigned int from_idx = 0; from_idx < 2; from_idx++)
        {
            cxutil::optional<ClosestPolygonPoint> from_opt = (from_idx == 0) ? from_a : from_b;
            if (!from_opt)
            {
                continue;
            }
            ...
            {
                cxutil::optional<ClosestPolygonPoint> to_opt = (to_idx == 0) ? to_a : to_b;
