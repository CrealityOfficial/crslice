...
namespace cxutil 
{
    ...
    void PolygonUtils::spreadDots(PolygonsPointIndex start, PolygonsPointIndex end, unsigned int n_dots, std::vector<ClosestPolygonPoint*>& result)
    {
        ...
        if (start == end)
        {
            result.emplace_back(new ClosestPolygonPoint(start.p(), start.point_idx, poly));
        ...
        while (true)
        {
            ...
            {
                result.emplace_back(new ClosestPolygonPoint(p0 + normal(p0p1, dist_past_vert_to_insert_point), vert.point_idx, poly));
                n_points_generated++;
            }
    ...
    {
        cxutil::optional<ClosestPolygonPoint> closest_polygon_point;
        if (loc_to_line_grid)
        {
     ...
     *
     */
    cxutil::optional<ClosestPolygonPoint> PolygonUtils::findClose(
        ...
        const std::function<int(Point)>& penalty_function)
    {
        ...
        {
            return cxutil::optional<ClosestPolygonPoint>();
        }
        else
        {
            bool bs_arg = true; // doesn't mean anything. Just to make clear we call the variable arguments of the constructor.
            return cxutil::optional<ClosestPolygonPoint>(bs_arg, best, best_point_poly_idx.point_idx, polygons[best_point_poly_idx.poly_idx], best_point_poly_idx.poly_idx);
        }
    }
        ...
        const std::function<int(Point)>& penalty_function)
    {
        ...
        for (unsigned int p1_idx = 0; p1_idx < from.size(); p1_idx++)
        {
            ...
            cxutil::optional<ClosestPolygonPoint> best_here = findClose(p1, destination, destination_loc_to_line, penalty_function);
            if (best_here)
            {
            ...
            for (unsigned int middle_point_nr = 1; dist_to_p1 > grid_size * 2; ++middle_point_nr)
            {
                ...
                cxutil::optional<ClosestPolygonPoint> best_here = findClose(x, destination, destination_loc_to_line, penalty_function);
                if (best_here)
                {
    ...
    cxutil::optional<ClosestPolygonPoint> PolygonUtils::getNextParallelIntersection(const ClosestPolygonPoint& start, const Point& line_to, const coord_t dist, const bool forward)
    {
        // <--o--t-----y----< poly 1
        //       :     :
        // >---o :====>:shift
