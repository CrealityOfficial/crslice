...
#define SGI_THIS SparseGrid<ElemT>
    ...
    SGI_TEMPLATE
        typename cxutil::Point SGI_THIS::toLowerCorner(const GridPoint& location)  const
    {
        return cxutil::Point(toLowerCoord(location.X), toLowerCoord(location.Y));
    }

    SGI_TEMPLATE
        typename cxutil::coord_t SGI_THIS::toLowerCoord(const grid_coord_t& grid_coord)  const
    {
        // This mapping via truncation results in the cells with
        // GridPoint.x==0 being twice as large and similarly for
        // GridPoint.y==0.  This doesn't cause any incorrect behavior,
