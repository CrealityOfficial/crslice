...
bool ArcFitter::do_arc_fitting(const Points& points, std::vector<PathFittingData>& result, double tolerance)
{
...
bool ArcFitter::do_arc_fitting_and_simplify(Points& points, std::vector<PathFittingData>& result, double tolerance)
{
    bool arcFittingValiable = false;
    //BBS: 1 do arc fit first
    if (abs(tolerance) > DEFAULT_SCALED_MIN_TOLERANCE)
        arcFittingValiable=ArcFitter::do_arc_fitting(points, result, tolerance);
    else
    ...
    if (result.size() == 1 && result[0].path_type == EMovePathType::Linear_move) {
        ...
        result[0].end_point_index = points.size() - 1;//强制保存最后点位置,保持路径的链接性
        return arcFittingValiable;
    } else {
