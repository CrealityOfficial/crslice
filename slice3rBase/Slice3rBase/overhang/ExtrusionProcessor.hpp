...
class ExtrusionQualityEstimator
{
    ...
    const AABBTreeLines::LinesDistancer<Linef> prev_layer_boundaries;
    const AABBTreeLines::LinesDistancer<Linef> next_layer_boundaries;
    //const PrintObject                                                            *current_object;

public:
    //void set_current_object(const PrintObject *object) { current_object = object; }  
     
    //void prepare_for_new_layer(const PrintObject * obj, const Layer *layer)  work gcode.cpp L2951
    //{
    //    if (layer == nullptr) return;
    //    const PrintObject *object = obj;
    //    prev_layer_boundaries[object] = next_layer_boundaries[object];
    //    next_layer_boundaries[object] = AABBTreeLines::LinesDistancer<Linef>{ Slic3r::to_unscaled_linesf(layer->lslices)};
    //}

    std::vector<ProcessedPoint> estimate_extrusion_quality(const  Slic3r::ClipperLib::Path& inputs,
                                                           ...
                                                           float                               original_speed)
    {
        ...
        for (size_t i = 0; i < speed_sections_count; i++) {
            float distance = pathWidth * (1.0 - (overlaps.at(i) / 100.0));
            //float speed    = speeds.at(i).percent ? (ext_perimeter_speed * speeds.get_at(i).value / 100.0) : speeds.get_at(i).value;
            float speed = 60.;
        ...
        //std::vector<Slic3r::ClipperLib:: IntPoint> pathXX;
        
        //Slic3r::Points  ps;
        //Slic3r::ClipperLib::Path inputs2;
        std::vector<ExtendedPoint> extended_points =
            estimate_points_properties<true, true, true, true>(inputs , prev_layer_boundaries, pathWidth);
        const auto width_inv = 1.0f / pathWidth;
