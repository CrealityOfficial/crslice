...
void GCodeExport::preSetup(const size_t start_extruder)
{
    ...
    if (scene.settings.get<bool>("klipper_time_estimate_enable"))
        estimateCalculator.reset(new TimeEstimateKlipper()); //?μ???????????
    //else 
...
void GCodeExport::writeGcodeHead()
{
    ...
    std::string version = "Creality Print Version : " + application->scene->settings.get<std::string>("software_version");
...
std::string GCodeExport::getFileHeader(const std::vector<bool>& extruder_is_used, const Duration* print_time, const std::vector<double>& filament_used, const std::vector<std::string>& mat_ids)
{
    ...
    case EGCodeFlavor::GRIFFIN:
        ...
        for (size_t extr_nr = 0; extr_nr < extruder_count; extr_nr++)
        {
            ...
            const Settings& extruder_settings = application->scene->extruders[extr_nr].settings;
        ...
        prefix << ";PRINT.GROUPS:" << application->scene->mesh_groups.size() << new_line;
    ...
    default:
        ...
        if (flavor == EGCodeFlavor::ULTIGCODE)
        {
            ...
            prefix << ";NOZZLE_DIAMETER:" << application->scene->extruders[0].settings.get<double>("machine_nozzle_size") << new_line;
        }
        else if (flavor == EGCodeFlavor::REPRAP || flavor == EGCodeFlavor::MARLIN || flavor == EGCodeFlavor::MARLIN_VOLUMATRIC || flavor == EGCodeFlavor::MACH3_Creality || flavor == EGCodeFlavor::PLC)
        {
            ...
            prefix << new_line;
            prefix << ";Layer Height:" << application->scene->current_mesh_group->settings.get<double>("layer_height") << new_line;
        }
        else
            prefix << ";Layer Height:" << application->scene->current_mesh_group->settings.get<double>("layer_height") << new_line;
							     ...
							     const std::vector<std::string>& mat_ids)
{
    ...
    if (extruder_count)
    {
        const Settings& settings = application->scene->extruders[0].settings;
...
Point GCodeExport::getGcodePos(const coord_t x, const coord_t y, const int extruder_train) const
{
    ...
    {
        const Settings& extruder_settings = application->scene->extruders[extruder_train].settings;
...
double GCodeExport::getCurrentExtrudedVolume() const
{
    ...
    const Settings& extruder_settings = application->scene->extruders[current_extruder].settings;
...
void GCodeExport::writeTravel(const coord_t x, const coord_t y, const coord_t z, const Velocity& speed)
{
    ...
    const double layer_height = application->scene->current_mesh_group->settings.get<double>("layer_height");
...
void GCodeExport::writeZhopStart(const coord_t hop_height, Velocity speed /*= 0*/)
{
    if (hop_height > 0)
    {
        if (speed == 0)
        {
            const ExtruderTrain& extruder = application->scene->extruders[current_extruder];
...
void GCodeExport::startExtruder(const size_t new_extruder)
{
    ...
    /*const*/ std::string start_code = application->scene->extruders[new_extruder].settings.get<std::string>("machine_extruder_start_code");
...
void GCodeExport::switchExtruder(size_t new_extruder, const RetractionConfig& retraction_config_old_extruder, coord_t perform_z_hop /*= 0*/)
{
    ...
    const Settings& old_extruder_settings = application->scene->extruders[current_extruder].settings;
...
bool GCodeExport::substitution(std::string& srcStr, const size_t start_extruder_nr)
{
    ...
    for (std::string& aStr : Vctdest)
    {
        if (!aStr.empty() && application->scene->extruders[start_extruder_nr].settings.has(aStr))
        {
            float value =application->scene->extruders[start_extruder_nr].settings.get<double>(aStr);
...
void GCodeExport::writeCdsFanCommand(double cds_speed)
{
    ...
    const bool should_scale_zero_to_one = application->scene->settings.get<bool>("machine_scale_fan_speed_zero_to_one");
...
{
    if (!application->scene->settings.get<bool>("machine_chamber_fan_exist")
...
void GCodeExport::writeTemperatureCommand(const size_t extruder, const Temperature& _temperature, const bool wait)
{
    ...
    const ExtruderTrain& extruder_train = application->scene->extruders[extruder];
...
{
    const ExtruderTrain& extruder = application->scene->extruders[current_extruder];
    const Settings& groupSettings = application->scene->current_mesh_group->settings;
    bool machine_center_is_zero = application->scene->machine_center_is_zero;
