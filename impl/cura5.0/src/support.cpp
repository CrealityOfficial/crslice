...
{
    const coord_t support_line_width = storage.application->current_slice->scene.current_mesh_group->settings.get<coord_t>("support_line_width");
...
Polygons AreaSupport::join(const SliceDataStorage& storage, const Polygons& supportLayer_up, Polygons& supportLayer_this, const coord_t smoothing_distance)
{
    ...
    const Settings& infill_settings = storage.application->current_slice->scene.current_mesh_group->settings.get<ExtruderTrain&>("support_infill_extruder_nr").settings;
    ...
    if (conical_support)
    {
        ...
        for (size_t extruder_nr = 0; extruder_nr < storage.application->current_slice->scene.extruders.size(); extruder_nr++)
        {
            ...
            const ExtruderTrain& other_extruder = storage.application->current_slice->scene.extruders[extruder_nr];
...
void AreaSupport::generateOverhangAreas(SliceDataStorage& storage)
{
    ...
    auto bSharpTail = [&](std::vector<Polygons>& upperMeshOutline, const Polygon& miniSupport, int layerIdx)
    {
        ...
        supportPartBoundary.expand(MM2INT(min_overhang_expand));
            ...
            for (size_t layer_idx = 0; layer_idx < storage.print_layer_count; layer_idx++)
            {
                ...
                for (auto& overhang_area : overhang_areas[layer_idx])
                {
                    ...
                    if (std::fabs(overhang_poly.area()) < MM2_2INT(5.0) && bSharpTail(upperMeshOutline, overhang_poly, layer_idx))
                    {
                        ...
                        {
                            Polygons overhangOutline = overhang_poly.offset(MM2INT(std::max(expand, min_overhang_expand)));
                            overhangOutline = overhangOutline.difference(upperMeshOutline[k - layer_idx]);
...
void AreaSupport::generateOverhangAreasForMesh(SliceDataStorage& storage, SliceMeshStorage& mesh)
{
    ...
    const coord_t layer_height = storage.application->current_slice->scene.current_mesh_group->settings.get<coord_t>("layer_height");
                                              ...
                                              std::vector<Polygons>& support_areas)
{
    ...
    for (size_t layer_idx = layer_count - 1 - layer_z_distance_top; layer_idx != static_cast<size_t>(-1); layer_idx--)
    {
        ...
        storage.application->progressor.messageProgress(Progress::Stage::SUPPORT, layer_count * (mesh_idx + 1) - layer_idx, layer_count * storage.meshes.size());
    }
