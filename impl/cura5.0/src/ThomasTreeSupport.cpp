...
#include <fstream>
#include <optional>
//#include <spdlog/spdlog.h> // Link wrong
#include <stdio.h>
#include <string>
#include <thread>
...
void ThomasTreeSupport::generateInitialAreas(const SliceMeshStorage& mesh, std::vector<std::set<TreeSupportElement*>>& move_bounds, SliceDataStorage& storage)
{
    ...
    const coord_t support_tree_branch_distance = mesh.settings.get<bool>("support_roof_enable") ? (support_pattern == EFillMethod::TRIANGLES ? 3 : (support_pattern == EFillMethod::GRID ? 2 : 1)) * connect_length : mesh.settings.get<coord_t>("support_tree_branch_distance");
