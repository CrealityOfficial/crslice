...
bool loadMeshSTL_binary(Mesh* mesh, const char* filename, const FMatrix4x3& matrix)
{
    ...
    {
        LOGW("Face count reported by file ({}) is not equal to actual face count ({}). File could be corrupt!", reported_face_count, face_count);
    }
...
bool loadMeshIntoMeshGroup(MeshGroup* meshgroup, const char* filename, const FMatrix4x3& transformation, Settings& object_parent_settings)
{
    ...
    LOGW("Unable to recognize the extension of the file. Currently only .stl and .STL are supported.");
    return false;
}
