...
#include "cxutil/infill/ZigzagConnectorProcessor.h"
#include "cxutil/settings/EnumSettings.h" //For infill types.
...
private:
    ...
    std::vector< InfillLineSegment*> uniform_on_line;
    ...
    void deleteInfillLineSegment(InfillLineSegment* seg);
    /*!
    ...
    void addLineInfillOneline(Polygons& result, const PointMatrix& rotation_matrix, const int scanline_min_idx, const int line_distance, const AABB boundary, std::vector<std::vector<coord_t>>& cut_list, coord_t shift, const SliceMeshStorage& mesh, Polygons& outline);
    void generateLinearBasedInfillOnewall(const int outline_offset, Polygons& result, const int line_distance, const PointMatrix& rotation_matrix, ZigzagConnectorProcessor& zigzag_connector_processor, const bool connected_zigzags, coord_t extra_shift, const SliceMeshStorage& mesh);
    void generateLineInfillOnewall(Polygons& result, int line_distance, const double& infill_rotation, coord_t shift, const SliceMeshStorage& mesh);
    void generateOnewallInfill(Polygons& result, const SliceMeshStorage& mesh);
