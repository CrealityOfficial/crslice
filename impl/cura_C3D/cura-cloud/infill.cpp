...
#include "cxutil/infill/GyroidInfill.h"
#include "cxutil/infill/NoZigZagConnectorProcessor.h"
#include "cxutil/infill/OneWallConnectorProcessor.h"
#include "cxutil/infill/SierpinskiFill.h"
#include "cxutil/infill/SierpinskiFillProvider.h"
#include "infill/SubDivCube.h"
#include "cxutil/infill/UniformDensityProvider.h"
#include "cxutil/util/PolygonConnector.h"
#include "cxutil/util/polygonUtils.h"
#include "cxutil/util/UnionFind.h"
...
void Infill::_generate(Polygons& result_polygons, Polygons& result_lines, const SierpinskiFillProvider* cross_fill_provider, const SliceMeshStorage* mesh)
{
    ...
    case EFillMethod::TRIANGLES:
        ...
        break;
    case EFillMethod::ONEWALL:
        generateOnewallInfill(result_lines, *mesh);
        break;
    ...
    for (InfillLineSegment* ils : uniform_on_line)
        delete ils;
    
    crossings_on_line.clear();
    uniform_on_line.clear();
}
...
void Infill::generateHoneycombInfill(Polygons& result, int _line_distance)
{
    ...
    infill_overlap;
    Polygons outline = in_outline.offset(-infill_line_width / 2 + infill_overlap);
    ...
    clipper.Clear();
    // add polygons
    clipper.AddPaths(all_polylines.getPaths(), ClipperLib::ptSubject, true);
    clipper.AddPaths(outline.getPaths(), ClipperLib::ptClip, true);
    // perform operation
...
void Infill::generateOnewallInfill(Polygons& result, const SliceMeshStorage& mesh)
{
    generateLineInfillOnewall(result, line_distance, fill_angle + 90, 0, mesh);
}
...
void Infill::addLineInfillOneline(Polygons& result, const PointMatrix& rotation_matrix, const int scanline_min_idx, const int line_distance, const AABB boundary, std::vector<std::vector<coord_t>>& cut_list, coord_t shift, const SliceMeshStorage& mesh, Polygons& outline)
{
    auto compare_coord_t = [](const void* a, const void* b)
    {
        coord_t n = (*(coord_t*)a) - (*(coord_t*)b);
    ...
    OneWallConnectorProcessor proce(rotation_matrix, result);
    unsigned int scanline_idx = 0;
    for (coord_t x = scanline_min_idx * line_distance + shift; x < boundary.max.X; x += line_distance)
    {
        if (scanline_idx >= cut_list.size())
        ...
        for (unsigned int crossing_idx = 0; crossing_idx + 1 < crossings.size(); crossing_idx += 2)
        {
            ...
            //We have to create our own lines when they are not created by the method connectLines.
            if (scanline_idx == 0)
                proce.registerVertex(Point(x, crossings[crossing_idx]));

            if (!zig_zaggify || pattern == EFillMethod::ZIG_ZAG || pattern == EFillMethod::LINES)
            {
                result.addLine(rotation_matrix.unapply(Point(x, crossings[crossing_idx + 1])), rotation_matrix.unapply(Point(x, crossings[crossing_idx + 0])));

                coord_t xx = x + line_distance;
                if (xx > boundary.max.X)
                    xx = x;
                if (scanline_idx + 1 >= cut_list.size())
                {
                    continue;
                }
                std::vector<coord_t>& crossings2 = cut_list[scanline_idx + 1];

                if (crossings2.size() == 0)
                    continue;
                //result.addLine(rotation_matrix.unapply(Point(xx, crossings2[crossing_idx + 1])), rotation_matrix.unapply(Point(x, crossings[crossing_idx ])));



                //result.addLine(rotation_matrix.unapply(Point(xx, crossings2[crossing_idx + 1])), rotation_matrix.unapply(Point(x, crossings[crossing_idx])));
                //if (scanline_idx < int(cut_list.size() /2)-1)



                proce.registerScanlineSegmentIntersection(Point(x, crossings[crossing_idx]), (Point(xx, crossings2.at(0))), (Point(xx, crossings2.at(crossings2.size() - 1))), outline, scanline_idx);
                //proce.registerScanlineSegmentIntersection((Point(xx, crossings2[crossing_idx + 0])), scanline_idx);


                //if (scanline_idx == int(cut_list.size() / 2))
                //    result.addLine(rotation_matrix.unapply(Point(x, crossings[crossing_idx + 1])), rotation_matrix.unapply(Point(x, crossings[crossing_idx + 0])));
                //if (scanline_idx >= int(cut_list.size() / 2)-1)
                //proce.registerScanlineSegmentIntersection((Point(xx, crossings2[crossing_idx + 1])), (Point(x, crossings[crossing_idx +0])), scanline_idx); 


                //proce.registerVertex(Point(xx, crossings2[crossing_idx + 1]));

                //crossings2.clear();
            }
            //proce.registerVertex(Point(x, crossings[crossing_idx + 0]));
...
void Infill::generateLineInfillOnewall(Polygons& result, int line_distance, const double& infill_rotation, coord_t shift, const SliceMeshStorage& mesh)
{
    shift = 14000;
    PointMatrix rotation_matrix(45 + 90);
    NoZigZagConnectorProcessor lines_processor(rotation_matrix, result);
    bool connected_zigzags = false;
    generateLinearBasedInfillOnewall(outline_offset, result, line_distance, rotation_matrix, lines_processor, connected_zigzags, shift, mesh);
}
...
void Infill::generateLinearBasedInfill(const int outline_offset, Polygons& result, const int line_distance, const PointMatrix& rotation_matrix, ZigzagConnectorProcessor& zigzag_connector_processor, const bool connected_zigzags, coord_t extra_shift)
{
    ...
    for (int scanline_index = min_scanline_index; scanline_index < max_scanline_index; scanline_index++)
    {
        ...
        for (long crossing_index = 0; crossing_index < static_cast<long>(crossings_per_scanline[scanline_index - min_scanline_index].size()) - 1; crossing_index += 2) //Combine each 2 subsequent crossings together.
        {
            ...
            uniform_on_line.push_back(new_segment);
        }
    }
...
void Infill::generateLinearBasedInfillOnewall(const int outline_offset, Polygons& result, const int line_distance, const PointMatrix& rotation_matrix, ZigzagConnectorProcessor& zigzag_connector_processor, const bool connected_zigzags, coord_t extra_shift, const SliceMeshStorage& mesh)
{
    if (line_distance == 0)
    {
        return;
    ...
    addLineInfillOneline(result, rotation_matrix, scanline_min_idx, line_distance, boundary, cut_list, shift, mesh, outline);
}
...
void Infill::connectLines(Polygons& result_lines)
{
    ...
    for (InfillLineSegment* infill_line : connected_lines)
    {
        ...
        //delete old_line;
        deleteInfillLineSegment(old_line);
...
void Infill::deleteInfillLineSegment(InfillLineSegment* seg)
{
