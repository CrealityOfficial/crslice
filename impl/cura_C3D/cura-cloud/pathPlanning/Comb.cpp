...
bool Comb::calc(const ExtruderTrain& extruder, Point startPoint, Point endPoint, CombPaths& combPaths, bool _startInside, bool _endInside, coord_t max_comb_distance_ignored)
{
    ...
    memset(&start_part_boundary_poly_idx, 0, sizeof(start_part_boundary_poly_idx));
    memset(&end_part_boundary_poly_idx, 0, sizeof(end_part_boundary_poly_idx));
    ...
    const bool perform_z_hops = extruder.settings->get<bool>("retraction_hop_enabled");
    const bool perform_z_hops_only_when_collides = extruder.settings->get<bool>("retraction_hop_only_when_collides");
    ...
    for (const ExtruderTrain& train : storage.scene->extruders)
    {
        travel_avoid_other_parts |= extruder_is_used[train.extruder_nr] && extruder.settings->get<bool>("travel_avoid_other_parts");
    }
...
bool Comb::Crossing::findOutside(const Polygons& outside, const Point close_to, const bool fail_on_unavoidable_obstacles, Comb& comber)
{
    ...
    { // move outside
        ...
        cxutil::optional<ClosestPolygonPoint> crossing_1_out_cpp = PolygonUtils::findClose(in_or_mid, outside, comber.getOutsideLocToLine(), close_to_penalty_function);
