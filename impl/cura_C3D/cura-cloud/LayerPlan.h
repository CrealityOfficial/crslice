...
#include "cxutil/settings/FanSpeedLayerTime.h"
#include "gcodeExport.h"
#include "pathOrderOptimizer.h"
#include "cxutil/settings/SpaceFillType.h"
#include "pathPlanning/GCodePath.h"
#include "pathPlanning/NozzleTempInsert.h"
#include "cxutil/progress/TimeMaterialEstimates.h"
...
protected:
    ...
    cxutil::optional<double> extrusion_temperature; //!< The normal temperature for printing this extruder plan. That start and end of this extruder plan may deviate because of the initial and final print temp (none if extruder plan has no extrusion moves)
    cxutil::optional<std::list<NozzleTempInsert>::iterator> extrusion_temperature_command; //!< The command to heat from the printing temperature of this extruder plan to the printing temperature of the next extruder plan (if it has the same extruder).
    cxutil::optional<double> prev_extruder_standby_temp; //!< The temperature to which to set the previous extruder. Not used if the previous extruder plan was the same extruder.
...
private:
    ...
    cxutil::optional<Point> last_planned_position; //!< The last planned XY position of the print head (if known)
    ...
    cxutil::optional<Point> first_travel_destination; //!< The destination of the first (travel) move (if this layer is not empty)
...
public:
    ...
    Ratio getLatestPathSpeedFactor();
...
public:
     ...
     */
    cxutil::optional<std::pair<Point, bool>> getFirstTravelDestinationState() const;
    ...
    */
     ...
     */
    void addLinesByOptimizer(const Polygons& polygons, const GCodePathConfig& config, SpaceFillType space_fill_type, bool enable_travel_optimization = false, int wipe_dist = 0, float flow_ratio = 1.0, cxutil::optional<Point> near_start_location = cxutil::optional<Point>(), double fan_speed = GCodePathConfig::FAN_SPEED_DEFAULT);
     ...
     * */
    void optimizePaths(const Point& starting_position, Scene* scene);
};
