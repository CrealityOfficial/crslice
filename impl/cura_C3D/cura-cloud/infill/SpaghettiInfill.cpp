...
#include "cxutil/settings/AngleDegrees.h" //For the infill angle.
#include "cxutil/settings/AngleRadians.h" //For the infill angle.
...
void SpaghettiInfill::generateTotalSpaghettiInfill(SliceMeshStorage& mesh)
{
    ...
    { // there is no infill in the whole mesh!
        //long long logSortId = mesh.settings->get<long long>("slice_log_sort");
       std::string sortStr = (mesh.settings->get<std::string>("slice_log_sort"));
       long long sortId = atoll(sortStr.c_str());
        LOGE(sortId,"Spaghetti Infill mesh doesn't have any volume!\n");
        return;
    }
