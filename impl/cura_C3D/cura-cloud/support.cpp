...
#include "cxutil/settings/EnumSettings.h" //For EFillMethod.
#include "cxutil/settings/AngleRadians.h" //To compute overhang distance from the angle.
...
void AreaSupport::splitGlobalSupportAreasIntoSupportInfillParts(SliceDataStorage& storage, const std::vector<Polygons>& global_support_areas_per_layer, unsigned int total_layer_count)
{
    ...
    const Settings& mesh_group_settings = *storage.scene->current_mesh_group->settings;
...
{
    const coord_t support_line_width = storage.scene->current_mesh_group->settings->get<coord_t>("support_line_width");
...
Polygons AreaSupport::join(const SliceDataStorage& storage, const Polygons& supportLayer_up, Polygons& supportLayer_this, const coord_t smoothing_distance)
{
    ...
    const Settings& infill_settings = storage.scene->current_mesh_group->settings->get<ExtruderTrain&>("support_infill_extruder_nr").settings;
    const AngleRadians conical_support_angle = infill_settings.get<AngleRadians>("support_conical_angle");//锥形支撑角度
    ...
    if (conical_support)//锥形支撑
    {
        ...
        for (size_t extruder_nr = 0; extruder_nr < storage.scene->extruders.size(); extruder_nr++)
        {
            ...
            const ExtruderTrain& other_extruder = storage.scene->extruders[extruder_nr];
    ...
    const coord_t join_distance = infill_settings.get<coord_t>("support_join_distance");//支撑结构间在 X/Y 方向的最大距离。当分离结构之间的距离小于此值时，这些结构将合并为一体
    if (join_distance > 0)
    {
...
void AreaSupport::generateOverhangAreas(SliceDataStorage& storage)
{
    for (SliceMeshStorage& mesh : storage.meshes)
    {
        ...
        {
            continue;//作为填充的mesh和作为防悬的mesh,过滤掉，不需要支撑，只有作为模型的mesh才生成支撑
        }
...
void AreaSupport::generateSupportAreas(SliceDataStorage& storage)
{
    ...
    for (unsigned int mesh_idx = 0; mesh_idx < storage.meshes.size(); mesh_idx++)
    {
        ...
        {
            if (minimum_support_area > 0.0 && !mesh.settings->get<bool>("support_mesh"))//手动支撑的支撑柱大小不需要过滤
            {
    ...
    for (unsigned int layer_idx = 0; layer_idx < storage.print_layer_count ; layer_idx++)
    {
        ...
        support_areas = support_areas.unionPolygons();//将有重叠的支撑轮廓合并？
    }
...
void AreaSupport::precomputeCrossInfillTree(SliceDataStorage& storage)
{
    ...
    if (support_pattern == EFillMethod::CROSS || support_pattern == EFillMethod::CROSS_3D)
    {
        ...
        for (unsigned int mesh_idx = 0; mesh_idx < storage.meshes.size(); mesh_idx++)
        {
            ...
            cxutil::AABB3D aabb_here(mesh.bounding_box);
        ...
        else
        {
            ...
            {
                LOGE(storage.scene->m_sliceLogSortId,"Cannot find density image \'%s\'.", cross_subdisivion_spec_image_file.c_str());
            }
...
void AreaSupport::generateOverhangAreasForMesh(SliceDataStorage& storage, SliceMeshStorage& mesh)
{
    ...
    {
        return;//如果mesh本身就是support_mesh，则不需要再生成支撑了
    }

    //Don't generate overhang areas if the Z distance is higher than the objects we're generating support for.
    const coord_t layer_height = storage.scene->current_mesh_group->settings->get<coord_t>("layer_height");
    const coord_t z_distance_top = mesh.settings->get<coord_t>("support_top_distance");//支撑顶部到打印品的距离
    const size_t z_distance_top_layers = round_up_divide(z_distance_top, layer_height) + 1; //支撑顶部到打印品的距离的层数 Support must always be 1 layer below overhang.
    ...
    if (use_towers)
    {
        AreaSupport::detectOverhangPoints(storage, mesh);//使用专门的塔来支撑较小的悬垂区域,这些塔的直径比它们所支撑的区域要大。在靠近悬垂物时塔的直径减小，形成顶板
    }
    ...
    {
        std::pair<Polygons, Polygons> basic_and_full_overhang = computeBasicAndFullOverhang(storage, mesh, layer_idx);//确认每层的悬垂轮廓
...
void AreaSupport::generateSupportAreasForMesh(SliceDataStorage& storage, const Settings& infill_settings, const Settings& roof_settings, const Settings& bottom_settings, const size_t mesh_idx, const size_t layer_count, std::vector<Polygons>& support_areas)
{
	...
	const bool is_support_enable = mesh.settings->get<bool>("support_enable");
	if (!is_support_enable && !is_support_mesh_place_holder)
	{
        return;
    }
    const Settings& mesh_group_settings = *storage.scene->current_mesh_group->settings;
    ...
    const size_t layer_z_distance_top = round_up_divide(z_distance_top, layer_thickness) + 1; //支撑顶部到打印品的距离的层数 support must always be 1 layer below overhang
    ...
    std::vector<Polygons> xy_disallowed_per_layer;//计算X/Y距离上不允许生成支撑的区域。  
    ...
    const coord_t xy_distance = infill_settings.get<coord_t>("support_xy_distance");//支撑结构在 X/Y 方向距打印品的距离
    const coord_t xy_distance_overhang = infill_settings.get<coord_t>("support_xy_distance_overhang");//支撑结构在 X/Y 方向距悬垂的距离
    //支撑距离优先级:支撑 X/Y 距离是否覆盖支撑 Z 距离或反之。当X/Y覆盖Z时:X/Y 距离可将支撑从模型上推离,影响与悬垂之间的实际 Z 距离。可以通过不在悬垂周围应用 X/Y 距离来禁用此选项
...
//#endif // defined(__GNUC__) && __GNUC__ <= 8
    ...
    const coord_t bottom_stair_step_width = std::max(static_cast<coord_t>(0), mesh.settings->get<coord_t>("support_bottom_stair_step_width"));//支撑梯步阶最大宽度
    const coord_t extension_offset = infill_settings.get<coord_t>("support_offset");//支撑水平扩展，生成更牢固的支撑

    const coord_t max_tower_supported_diameter = infill_settings.get<coord_t>("support_tower_maximum_supported_diameter");//支撑塔最大直径
    const bool use_towers = infill_settings.get<bool>("support_use_towers") && max_tower_supported_diameter > 0;//使用支撑塔

    coord_t smoothing_distance;
    { // compute best smoothing_distance
        const ExtruderTrain& infill_extruder = mesh_group_settings.get<ExtruderTrain&>("support_infill_extruder_nr");
        const coord_t infill_line_width = infill_extruder.settings->get<coord_t>("support_line_width");
        ...
        if (mesh.settings->get<bool>("support_roof_enable"))
        {
            ...
            const coord_t roof_line_width = roof_extruder.settings->get<coord_t>("support_roof_line_width");
        ...
        if (mesh.settings->get<bool>("support_bottom_enable"))
        {
            ...
            const coord_t bottom_line_width = bottom_extruder.settings->get<coord_t>("support_bottom_line_width");
            smoothing_distance = std::max(smoothing_distance, bottom_line_width);
        }
    }

    const coord_t z_distance_bottom = ((mesh.settings->get<bool>("support_bottom_enable")) ? bottom_settings : infill_settings).get<coord_t>("support_bottom_distance");//从打印品到支撑底部的距离
    const size_t bottom_empty_layer_count = round_up_divide(z_distance_bottom, layer_thickness); //从打印品到支撑底部的距离的层数 number of empty layers between support and model
    const coord_t bottom_stair_step_height = std::max(static_cast<coord_t>(0), mesh.settings->get<coord_t>("support_bottom_stair_step_height"));//支撑梯步阶高度
    ...
    for (size_t layer_idx = layer_count - 1 - layer_z_distance_top; layer_idx != static_cast<size_t>(-1); layer_idx--)
    {
        ...
        { // join with support from layer up
            ...
            layer_this = AreaSupport::join(storage, *layer_above, layer_this, smoothing_distance);//当前层悬垂面积与上一层的支撑面积的并集
        }
        ...
        //模型上的支撑生成阶梯底部 Move up from model, while taking the (post-processed) x/y-disallowed area into account.
        ...
        storage.scene->progress.messageProgress(Progress::Stage::SUPPORT, layer_count * (mesh_idx + 1) - layer_idx, layer_count * storage.meshes.size());
    }

    // 裁剪掉不需要支撑的xy区域(xy_disallowed_per_layer) Substract x/y-disallowed area from the support.
...
//#endif // defined(__GNUC__) && __GNUC__ <= 8
        ...
        // 裁剪掉支撑面积与模型外轮廓重合的部分 Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
...
{
    Polygons supportLayer_supportee = mesh.layers[layer_idx].getOutlines();//当前层轮廓print_outline
    constexpr bool no_support = false;
    constexpr bool no_prime_tower = false;
    Polygons supportLayer_supporter = storage.getLayerOutlines(layer_idx-1, no_support, no_prime_tower);//前一层轮廓print_outline
    ...
    const coord_t max_dist_from_lower_layer = tan_angle * layer_height; //可以通过桥接打印而不需要支撑的悬垂宽度Maximum horizontal distance that can be bridged.
    Polygons supportLayer_supported =  supportLayer_supporter.offset(max_dist_from_lower_layer);//前一层轮廓加上桥接距离得到 前一层的实际的支撑轮廓
    Polygons basic_overhang = supportLayer_supportee.difference(supportLayer_supported);//当前层轮廓求异前一层实际支撑轮廓得到当前层的最少的支撑轮廓
...
//         presumably the computation above is slower than the one below
    ...
    Polygons full_overhang = overhang_extented.intersection(supportLayer_supportee);//忽略桥接宽度的充分支撑的轮廓
