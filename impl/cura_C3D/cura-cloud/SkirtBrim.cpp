...
void SkirtBrim::getFirstLayerOutline(SliceDataStorage& storage, const size_t primary_line_count, const bool is_skirt, const bool sliceBelt,Polygons& first_layer_outline)
{
    const ExtruderTrain& extruder = storage.scene->current_mesh_group->settings->get<ExtruderTrain&>("adhesion_extruder_nr");
    const ExtruderTrain& support_infill_extruder = storage.scene->current_mesh_group->settings->get<ExtruderTrain&>("support_infill_extruder_nr");
    const bool external_only = is_skirt || extruder.settings->get<bool>("brim_outside_only"); //Whether to include holes or not. Skirt doesn't have any holes.
    ...
    if (is_skirt)
    {
        ...
        if (sliceBelt)
        {
            first_layer_outline = storage.getLayerOutlinessliceBelt(layer_nr, include_support, include_prime_tower, external_only, sliceBelt);
            first_layer_outline = first_layer_outline.approxConvexHull();
        }
        else
        {
            first_layer_outline = storage.getLayerOutlines(layer_nr, include_support, include_prime_tower, external_only,sliceBelt);
            first_layer_outline = first_layer_outline.approxConvexHull();
        }
    ...
    { // add brim underneath support by removing support where there's brim around the model
        ...
        { // remove model-brim from support
            ...
            if (support_infill_extruder.settings->get<bool>("brim_replaces_support"))
            {
                ...
                //  |+-+|     |+--+|
                //  +---+     +----+
                const coord_t primary_extruder_skirt_brim_line_width = extruder.settings->get<coord_t>("skirt_brim_line_width") * extruder.settings->get<Ratio>("initial_layer_line_width_factor");
        ...
        if (storage.primeTower.enabled && !extruder.settings->get<bool>("prime_tower_brim_enable"))
        {
    ...
    {
        LOGE(storage.scene->m_sliceLogSortId,"Couldn't generate skirt / brim! No polygons on first layer.\n");
    }
}
...
{
    const Settings& adhesion_settings = storage.scene->current_mesh_group->settings->get<ExtruderTrain&>("adhesion_extruder_nr").settings;
...
void SkirtBrim::generate(SliceDataStorage& storage, Polygons first_layer_outline, const coord_t start_distance, size_t primary_line_count, const bool allow_helpers /*= true*/)
{
    ...
    if (scene->settings.get<EPlatformAdhesion>("adhesion_type") == cxutil::EPlatformAdhesion::LACE)
    {
    ...
    //        const coord_t width = extruder.settings->get<coord_t>("skirt_brim_line_width") * extruder.settings->get<Ratio>("initial_layer_line_width_factor");
    //        const coord_t minimal_length = extruder.settings->get<coord_t>("skirt_brim_minimal_length");
