...
public:
     ...
     */
    bool generateAreas(SliceDataStorage& storage, MeshGroup* object, cxutil::TimeKeeper* timeKeeper, std::vector<Polygons>& skirt_brims);
private:
    /*!
     ...
     */
    bool sliceModel(MeshGroup* object, cxutil::TimeKeeper* timeKeeper, SliceDataStorage& storage); /// slices the model
     ...
     * \param timeKeeper Object which keeps track of timings of each stage.
     */
    void slices2polygons(SliceDataStorage& storage, cxutil::TimeKeeper* timeKeeper, std::vector<Polygons>& skirt_brims);
  ...
  //  coord_t calacAdhesionSize(const EPlatformAdhesion& adhesion_type);
