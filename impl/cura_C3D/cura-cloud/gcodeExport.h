...
#include "cxutil/math/AABB3D.h" //To track the used build volume for the Griffin header.
#include "cxutil/util/timeEstimate.h"
#include "cxutil/settings/EnumSettings.h"
#include "cxutil/settings/Settings.h" //For MAX_EXTRUDERS.
#include "cxutil/settings/Temperature.h" //Bed temperature.
...
public:
    ...
    void writeOutputStream(const std::string dataString);
    ...
    void writeG12TopHeater(const Temperature& temperature);
    ...
    void updateTotalGcodeLayers() {total_gen_gcode_layers++; }
    size_t getTotalGcodeLayers() { return total_gen_gcode_layers; }
...
public:
    ...
    void writeBedTemperatureCommand(const Temperature& temperature, std::vector<bool>& vctHotZone ,const bool wait = false);
    ...
    std::string getPatternTypeStr(EFillMethod type);
    void writeGcodeHead();
