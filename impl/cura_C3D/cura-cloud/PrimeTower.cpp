...
PrimeTower::PrimeTower(Scene* _scene)
...
, scene(_scene)
{
    ...
    {
        const Ratio adhesion_a = scene->extruders[extruder_nr_a].settings->get<Ratio>("material_adhesion_tendency");
        const Ratio adhesion_b = scene->extruders[extruder_nr_b].settings->get<Ratio>("material_adhesion_tendency");
...
void PrimeTower::generatePaths_denseInfill()
{
    ...
    for (size_t extruder_nr : extruder_order)
    {
		...
		if (extruder_nr % 2 == 1)
		{
            pattern_layer0.polygons.add(polygons2);
		}
        else
        {
			...
			//if (extruder_nr%2==1)
			//{
			//    inset = outer_poly.offset(line_width_layer0/2);
			//}
            ...
            int icount = pattern_layer0.polygons.size() / 1.5;
            for (int n=0;n< icount;n++)
            {
                polygons2.add(pattern_layer0.polygons[pattern_layer0.polygons.size()-1]);
                pattern_layer0.polygons.remove(pattern_layer0.polygons.size() - 1);
            }
        }
    }
}
...
void PrimeTower::addToGcode(const SliceDataStorage& storage, LayerPlan* gcode_layer, const int prev_extruder, const int new_extruder) const
{
    ...
    bool post_wipe = scene->extruders[prev_extruder].settings->get<bool>("prime_tower_wipe_enabled");
    ...
    if (post_wipe)
    {
        ...
        const Settings& previous_settings = scene->extruders[prev_extruder].settings;
        ...
        const Settings& new_settings = scene->extruders[new_extruder].settings;
...
{
    const ExtrusionMoves& pattern = (gcode_layer->getLayerNr() == -static_cast<LayerIndex>(Raft::getFillerLayerCount(scene)))
...
void PrimeTower::gotoStartLocation(LayerPlan* gcode_layer, const int extruder_nr) const
{
    ...
    const coord_t inward_dist = extruder.settings->get<coord_t>("machine_nozzle_size") * 3 / 2 ;
    const coord_t start_dist = extruder.settings->get<coord_t>("machine_nozzle_size") * 2;
