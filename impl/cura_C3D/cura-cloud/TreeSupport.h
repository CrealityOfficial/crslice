...
private:
     ...
     */
    static Polygons calculateMachineBorderCollision(cxutil::Polygon machine_border);
...
private:
     ...
     * vector of layers.
     */
    void dropNodes(SliceDataStorage& storage, std::vector<std::unordered_set<Node*>>& contact_nodes);
...
namespace std
{
    template<> struct hash<cxutil::TreeSupport::Node>
    {
        size_t operator()(const cxutil::TreeSupport::Node& node) const
        {
            return hash<cxutil::Point>()(node.position);
        }
    };
}
