...
#include "cxutil/infill/ImageBasedDensityProvider.h"
#include "cxutil/infill/SierpinskiFillProvider.h"
#include "infill/SpaghettiInfill.h"
#include "infill/SubDivCube.h"
#include "cxutil/infill/UniformDensityProvider.h"
#include "progress/Progress.h"
#include "cxutil/progress/ProgressEstimator.h"
#include "cxutil/progress/ProgressEstimatorLinear.h"
#include "cxutil/progress/ProgressStageEstimator.h"
#include "cxutil/settings/AdaptiveLayerHeights.h"
#include "cxutil/settings/LayerIndex.h"
#include "cxutil/util/algorithm.h"

#include "cxutil/settings/AngleRadians.h"
...
namespace cxutil
{
    bool FffPolygonGenerator::generateAreas(SliceDataStorage& storage, MeshGroup* meshgroup, cxutil::TimeKeeper* timeKeeper, std::vector<Polygons>& skirt_brims)
    {
        ...
        slices2polygons(storage, timeKeeper, skirt_brims);
    ...
    bool FffPolygonGenerator::sliceModel(MeshGroup* meshgroup, cxutil::TimeKeeper* timeKeeper, SliceDataStorage& storage) /// slices the model
    {
        ...
        {
            LOGE(storage.scene->m_sliceLogSortId,"Initial layer height %i is disallowed.\n", initial_layer_thickness);
        ...
        {
            LOGE(storage.scene->m_sliceLogSortId, "Layer height %i is disallowed.\n", layer_thickness);
            return false;
        }
        ...
        if (use_variable_layer_heights)
        {
            ...
            adaptive_layer_heights = new AdaptiveLayerHeights(layer_thickness, variable_layer_height_max_variation,
                variable_layer_height_variation_step, adaptive_threshold, meshgroup);
        ...
        for (unsigned int mesh_idx = 0; mesh_idx < meshgroup->meshes.size(); mesh_idx++)
        {
            ...
            Slicer* slicer = new Slicer(mesh, layer_thickness, slice_layer_count,
                use_variable_layer_heights, adaptive_layer_height_values, meshgroup);

            //z扩展 mesh->settings->get<SlicingTolerance>("slicing_tolerance");
            if (mesh->settings->get<coord_t>("z_offset")> 0)
			{
				float vexpand = mesh_group_settings.get<coord_t>("z_offset");
				float height = mesh_group_settings.get<coord_t>("layer_height");
				int V_expand_layer = vexpand / height;

				//SlicedMesh& slicedMesh = slicedMeshes.at(i);
				std::vector<SlicerLayer> Oldlayers = slicer->layers;
				for (size_t j = 0; j < Oldlayers.size(); ++j)
				{
                    SlicerLayer& layer = slicer->layers.at(j);
					if (j >= V_expand_layer)
					{
                        SlicerLayer& layerCut = Oldlayers.at(j - V_expand_layer);
						layer.polygons = layer.polygons.unionPolygons(layerCut.polygons);
					}
				}
				for (int n = 0; n < V_expand_layer; n++)
				{
                    int preZ = slicer->layers.back().z;
                    slicer->layers.push_back(Oldlayers[Oldlayers.size() + n - V_expand_layer]);
                    slicer->layers.back().z = preZ+height;
				}
				////二次合并
				//std::vector<SlicerLayer> Oldlayers2 = slicer->layers;
				//int V_expand_layer2 = V_expand_layer;
				//for (size_t j = 0; j < slicer->layers.size(); ++j)
				//{
                //    SlicerLayer& layer = slicer->layers.at(j);
				//	if (j >= V_expand_layer2)
				//	{
                //        SlicerLayer& layerCut = Oldlayers2.at(j - V_expand_layer2);
				//		layer.polygons = layer.polygons.unionPolygons(layerCut.polygons);
				//	}
				//}
				//for (int n = 0; n < V_expand_layer2; n++)
				//{
				//	int preZ = slicer->layers.back().z;
				//	slicer->layers.push_back(Oldlayers[Oldlayers.size() - n - 1]);
				//	slicer->layers.back().z = preZ + height;
				//}
				////删除v第一个元素
				//for (int n = 0; n < V_expand_layer; n++)
				//{
				//	std::vector<SlicerLayer>::iterator k = slicer->layers.begin();
                //    slicer->layers.erase(k);
				//}
                //for (SlicerLayer& layer: slicer->layers)
                //{
                //    layer.z -= V_expand_layer*height;
                //}
			}

            slicerList.push_back(slicer);

            scene->progress.messageProgress(Progress::Stage::SLICING, mesh_idx + 1, meshgroup->meshes.size());
        }

        if (scene->debugCallback)
        {
            //preview
            for (int n = 0; n < slicerList[0]->layers.size(); n++)
            {
                scene->debugCallback->onZPaths(&slicerList[0]->layers[n].polygons.paths, slicerList[0]->layers[n].z, 7);
            }
        }
        ...
        Mold::process(slicerList, meshgroup);
        ...
        {
            const bool alternate_carve_order = scene->current_mesh_group->settings->get<bool>("alternate_carve_order");
            carveMultipleVolumes(slicerList, alternate_carve_order);
        }
        ...
        for (unsigned int meshIdx = 0; meshIdx < slicerList.size(); meshIdx++)
        {
            ...
            //optimize enure each part outline count
            int max_part_outline_num = mesh->settings->get<int>("max_part_outline_num");
            for (SliceLayer& sliceLayer : meshStorage.layers)
            {
                for (SliceLayerPart& layerPart:sliceLayer.parts)//
                {
                     layerPart.outline.optimize(max_part_outline_num);
                }
            }
            ...
            for (unsigned int layer_nr = 0; layer_nr < meshStorage.layers.size(); layer_nr++)
            {
                ...
                {
                    const ExtruderTrain& extruder = mesh_group_settings.get<ExtruderTrain&>("adhesion_extruder_nr");
                    auto t = Raft::getTotalSimpleRaftThickness(scene);
            ...
            scene->progress.messageProgress(Progress::Stage::PARTS, meshIdx + 1, slicerList.size());
        }
        ...
        if (scene->callback)
        {
            for (unsigned int meshIdx = 0; meshIdx < storage.meshes.size(); meshIdx++)
            {
                ...
                for (size_t layerIdx = 0; layerIdx < layerSize; ++layerIdx)
                {
                    ...
                    float z = (float)(layer.printZ) / 1000.0f;
                    float thickness = (float)(layer.thickness) / 1000.0f;

                    size_t partSize = layer.parts.size();
                    for (size_t partIdx = 0; partIdx < partSize; ++partIdx)
                    {
                        SliceLayerPart& part = layer.parts.at(partIdx);
                        cxutil::PolygonRef polygon = part.outline.outerPolygon();
                        scene->callback->onLayerPart((int)meshIdx, (int)layerIdx, (int)partIdx,
                            z, thickness, *polygon);
                    }
                }
            }
    ...
    void FffPolygonGenerator::slices2polygons(SliceDataStorage& storage, cxutil::TimeKeeper* time_keeper, std::vector<Polygons>& skirt_brims)
    {
        ...
        for (size_t mesh_order_idx = 0; mesh_order_idx < mesh_order.size(); ++mesh_order_idx)
        {
            ...
            scene->progress.messageProgress(Progress::Stage::INSET_SKIN, mesh_order_idx + 1, storage.meshes.size());
            if (scene->iFunc && scene->iFunc())
            {
                return;
            }
        ...
        if (scene->debugCallback)
        {
            std::vector<SliceLayer>& layers = storage.meshes[0].layers;
            for (int n = 0; n < layers.size(); n++)
            {
                ClipperLib::Paths printOutlinePaths;
                ClipperLib::Paths infillAreaPathss;
                ClipperLib::Paths skinPathss;
                for (std::vector<SliceLayerPart>::iterator it = layers[n].parts.begin(); it != layers[n].parts.end(); it++)
                {
                    for (ClipperLib::Path& apath : it->print_outline.paths)
                    {
                        printOutlinePaths.push_back(apath);
                    }
                    for (ClipperLib::Path& apath : it->infill_area)
                    {
                        infillAreaPathss.push_back(apath);
                    }
                    for (SkinPart& askinPart : it->skin_parts)
                    {
                        skinPathss.push_back(askinPart.outline.paths[0]);
                    }
                }

                scene->debugCallback->onZPaths(&printOutlinePaths, layers[n].printZ, 8);
                scene->debugCallback->onZPaths(&infillAreaPathss, layers[n].printZ, 9);
                scene->debugCallback->onZPaths(&skinPathss, layers[n].printZ, 10);
            }
        }
        const Settings& mesh_group_settings = *scene->current_mesh_group->settings;
       ...
       AreaSupport::generateOverhangAreas(storage);//生成模型每层的悬垂面积

       if (scene->iFunc && scene->iFunc())
       {
           return;
       }
        AreaSupport::generateSupportAreas(storage);//悬垂面积转换成每层支撑面积
        if (scene->iFunc && scene->iFunc())
        {
            return;
        }
        ...
        if (scene->debugCallback)
        {
            ...
            for (int n = 0; n < supportLayers.size(); n++)
            {
                ClipperLib::Paths supportPaths;
                ClipperLib::Paths roofPaths;
                ClipperLib::Paths bottomPaths;
                for (SupportInfillPart& supPart : supportLayers[n].support_infill_parts)
                {
                    for (ClipperLib::Path& apath : supPart.outline.paths)
                    {
                        supportPaths.push_back(apath);
                    }
                }
                roofPaths = supportLayers[n].support_roof.paths;
                bottomPaths = supportLayers[n].support_bottom.paths;

                scene->debugCallback->onZPaths(&supportPaths, layers[n].printZ, 4);
                scene->debugCallback->onZPaths(&roofPaths, layers[n].printZ, 5);
                scene->debugCallback->onZPaths(&bottomPaths, layers[n].printZ, 6);
            }
        }
        ...
        {
            LOGI("Stopping process because there are no non-empty layers.\n");
            return;
        }
        ...
        {
            LOGI("Processing platform adhesion\n");
            if (storage.scene->settings.get<bool>("machine_is_belt") && skirt_brims.size())
            {
                if (storage.scene->settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SKIRT 
                    || storage.scene->settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::BRIM
                    || storage.scene->settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::LACE)
                    {
                         for (int i = 0; i < storage.meshes.size(); i++)
                            storage.skirt_brim->add(skirt_brims[i]);                   
                    }
                if (storage.scene->settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT || storage.scene->settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SIMPLERAFT)
                    storage.raftOutline.add(skirt_brims[0]);
            }
            else
            {
        ...
        if (scene->callback)
        {
            ...
            for (size_t layerIdx = 0; layerIdx < layerSize; ++layerIdx)
            {
                ...
                for (size_t partIdx = 0; partIdx < partSize; ++partIdx)
                {
                    ...
                    cxutil::PolygonRef plogon = part.outline.outerPolygon();
                    paths.push_back(*plogon);

                    if (thickness<=0.0f)
                    {
                        thickness = (float)(part.support_line_width) / 1000.0f;
                    }
                    //cxutil::PolygonRef polygon = part.outline.outerPolygon();
                }
                if(paths.size())
                    scene->callback->onSupports((int)layerIdx, thickness, paths);
            }
        }
    }
...
#pragma omp parallel for default(none) shared(mesh_layer_count, storage, mesh, inset_skin_progress_estimate, processed_layer_count) schedule(dynamic)
        ...
        for (int layer_number = 0; layer_number < static_cast<int>(mesh.layers.size()); layer_number++)
        {
            ...
            //LOGD("Processing insets for layer %i of %i\n", layer_number, mesh_layer_count);
...
#pragma omp atomic read
                ...
                scene->progress.messageProgress(Progress::Stage::INSET_SKIN, progress * 100, 100);
            }
#pragma omp atomic
...
#pragma omp for schedule(dynamic)
            ...
            for (int layer_number = 0; layer_number < static_cast<int>(mesh.layers.size()); layer_number++)
            {
                ...
                //LOGD("Processing skins and infill layer %i of %i\n", layer_number, mesh_layer_count);
...
#pragma omp atomic
    ...
    void FffPolygonGenerator::processOutlineGaps(SliceDataStorage& storage)
    {
        for (SliceMeshStorage& mesh : storage.meshes)
        {
            ...
            for (unsigned int layer_nr = 0; layer_nr < mesh.layers.size(); layer_nr++)
            {
                ...
                if (layer_nr == 0)
                {
                    const ExtruderTrain& extruder = mesh.settings->get<ExtruderTrain&>("wall_0_extruder_nr");
                    Ratio initial_layer_line_width_factor = extruder.settings->get<Ratio>("initial_layer_line_width_factor");
    ...
    void FffPolygonGenerator::processPerimeterGaps(SliceDataStorage& storage)
    {
        ...
        for (SliceMeshStorage& mesh : storage.meshes)
        {
            ...
            const size_t hole_wall_line_count = mesh.settings->get<size_t>("hole_wall_line_count");
            if (!fill_perimeter_gaps || hole_wall_line_count < mesh.settings->get<size_t>("wall_line_count"))
            {
                continue;
            }
        ...
        else
        {
                ...
                )
            {
                ...
                else
                {
                    ...
                    {
                        LOGE(scene->m_sliceLogSortId, "Cannot find density image \'%s\'.", cross_subdisivion_spec_image_file.c_str());
                    }
    ...
    void FffPolygonGenerator::removeEmptyFirstLayers(SliceDataStorage& storage, size_t& total_layers)
    {
        ...
        {
            LOGI("Removing %d layers because they are empty\n", n_empty_first_layers);
            ...
            scene->current_mesh_group->settings->add("machine_belt_offset_Y", str);
...
//        NONE
//};
    const float skirt_brim_line_width = 0.4;
    const float initial_layer_line_width_factor = 1.0;
    const int brim_line_count = 20;
    const int skirt_line_count = 1;
    const float raft_margin = 15.0;
    const float skirt_gap = 3.0;
    //float  FffPolygonGenerator::calacAdhesionSize(const EPlatformAdhesion& adhesion_type)
    //{
    //    float adhesion_size = 0.0;
    //    switch (adhesion_type)
    //    {
    //    case EPlatformAdhesion::BRIM:
    //        adhesion_size = skirt_brim_line_width *initial_layer_line_width_factor * brim_line_count;
    //        break;
    //    case EPlatformAdhesion::RAFT:
    //        adhesion_size = raft_margin;
    //        break;
    //    case EPlatformAdhesion::SKIRT:
    //        adhesion_size = skirt_gap + skirt_brim_line_width*initial_layer_line_width_factor*skirt_line_count ;
    //        break;
    ...
    void FffPolygonGenerator::processSkinsAndInfill(SliceMeshStorage& mesh, const LayerIndex layer_nr, bool process_infill)
    {
        ...
        SkinInfillAreaComputation skin_infill_area_computation(layer_nr, mesh, process_infill,mesh.settings->get<bool>("infill_has_default_inset"));
    ...
    void FffPolygonGenerator::computePrintHeightStatistics(SliceDataStorage& storage)
    {
        ...
        max_print_height_per_extruder.resize(extruder_count, -(Raft::getTotalExtraLayers(scene) + 1)); //Initialize all as -1 (or lower in case of raft).
        ...
        {
            storage.max_print_height_second_to_last_extruder = -(Raft::getTotalExtraLayers(scene) + 1);
        }
    }
    ...
    void generate(FloatPaths& originPaths, FloatPaths& newPaths)
    {
    ...
    ClipperLib::Paths skirt2Lace(ClipperLib::Paths& outlinePaths)
    {
    ...
    {
        const Settings& mesh_group_settings = *scene->current_mesh_group->settings;
        ExtruderTrain& extruder = mesh_group_settings.get<ExtruderTrain&>("adhesion_extruder_nr");
        ...
        {
            primary_line_count = extruder.settings->get<size_t>("skirt_line_count");
            SkirtBrim::getFirstLayerOutline(storage, primary_line_count, true, false, first_layer_outline);
            SkirtBrim::generate(storage, first_layer_outline, extruder.settings->get<coord_t>("skirt_gap"), primary_line_count);
        }
        ...
        if (adhesion_type == EPlatformAdhesion::LACE)
        {
            ...
            ClipperLib::Paths pathsTemp;
            ClipperLib::Paths outlinePaths = first_layer_outline.getPaths();
            pathsTemp = skirt2Lace(outlinePaths);
            first_layer_outline.clear();
            first_layer_outline.setPaths(pathsTemp);
            SkirtBrim::generate(storage, first_layer_outline, extruder.settings->get<coord_t>("skirt_gap"), primary_line_count);
        }
        ...
        if (adhesion_type != EPlatformAdhesion::RAFT && adhesion_type != EPlatformAdhesion::SIMPLERAFT&& storage.primeTower.enabled && mesh_group_settings.get<bool>("prime_tower_brim_enable"))
        {
            ...
            SkirtBrim::generate(storage, storage.primeTower.outer_poly, 0, extruder.settings->get<size_t>("brim_line_count"), dont_allow_helpers);
        }
        ...
        case EPlatformAdhesion::SKIRT:
            ...
            break;
        case EPlatformAdhesion::BRIM:
            primary_line_count = extruder.settings->get<size_t>("brim_line_count");
            SkirtBrim::getFirstLayerOutline(storage, primary_line_count, false,false, first_layer_outline);
        ...
        const coord_t line_segment_resolution = extruder.settings->get<coord_t>("meshfix_maximum_resolution");
        const coord_t line_segment_deviation = extruder.settings->get<coord_t>("meshfix_maximum_deviation");
