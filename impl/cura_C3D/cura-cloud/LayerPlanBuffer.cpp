...
void LayerPlanBuffer::handle(LayerPlan* layer_plan, GCodeExport& gcode)
{
    ...
    if (to_be_written)
    {
        gcode.is_firstlayerSkirt = false;
        if (to_be_written->is_initial_layer && to_be_written->is_raft_layer)
            gcode.is_firstlayerSkirt = true;
...
LayerPlan* LayerPlanBuffer::processBuffer()
{
    ...
    if (buffer.size() > buffer_size)
    {
        ...
        scene->communication->flushGCode();
        buffer.pop_front();
        return ret;
    }
    return nullptr;
...
{
    scene->communication->flushGCode(); //If there was still g-code in a layer, flush that as a separate layer. Don't want to group them together accidentally.
    if (buffer.size() > 0)
    {
...
{
    cxutil::optional<std::pair<Point, bool>> new_layer_destination_state = newest_layer->getFirstTravelDestinationState();

    if (!new_layer_destination_state)
    {
        long long sortId = atoll(scene->settings.get<std::string>("slice_log_sort").c_str());
        LOGWID(sortId,"Layer %d is empty (or it has empty extruder plans). Temperature control and cross layer travel moves might suffer!\n", newest_layer->layer_nr);
        return;
    }
    ...
    if (!prev_layer->last_planned_position || *prev_layer->last_planned_position != first_location_new_layer)
    {
        ...
        const Settings& extruder_settings = scene->extruders[prev_layer->extruder_plans.back().extruder_nr].settings;
...
Preheat::WarmUpResult LayerPlanBuffer::computeStandbyTempPlan(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
    ...
    Settings& extruder_settings = *scene->extruders[extruder].settings;
...
{
    if (!scene->extruders[extruder_nr].settings->get<bool>("machine_nozzle_temp_enabled"))
    {
        return;
    }
...
void LayerPlanBuffer::handleStandbyTemp(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx, double standby_temp)
{
    ...
    LOGW("Warning: Couldn't find previous extruder plan so as to set the standby temperature. Inserting temp command in earliest available layer.\n");
...
void LayerPlanBuffer::insertPreheatCommand_multiExtrusion(std::vector<ExtruderPlan*>& extruder_plans, unsigned int extruder_plan_idx)
{
    ...
    const Settings& extruder_settings = scene->extruders[extruder].settings;
    if (!extruder_settings.get<bool>("machine_nozzle_temp_enabled"))
    {
        return;
    }

    //wangwenbin add 20220308  prev_extruder_standby_temp replace required_start_temperature
    double initial_print_temp = *extruder_plan.prev_extruder_standby_temp;;//extruder_plan.required_start_temperature;
...
{   
    ...
    { // set previous extruder to standby temperature
        const Settings& previous_extruder_settings = scene->extruders[prev_extruder].settings;
    ...
    else if (scene->extruders[extruder].settings->get<bool>("machine_extruders_share_heater"))
    {
...
void LayerPlanBuffer::insertFinalPrintTempCommand(std::vector<ExtruderPlan*>& extruder_plans, unsigned int last_extruder_plan_idx)
{
    ...
    cxutil::optional<double> initial_print_temp; // The initial print temp of the first extruder plan with this extruder
    ...
    for (size_t extruder_plan_idx = 0; extruder_plan_idx < layer_plan.extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        { // the very first extruder plan of the current meshgroup
            ...
            { // set temperature of the first nozzle, turn other nozzles down
                const Settings& other_extruder_settings = scene->extruders[extruder_idx].settings;
