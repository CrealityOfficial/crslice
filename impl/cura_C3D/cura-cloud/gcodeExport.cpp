...
#include "cxutil/util/string.h" // cxutil::MMtoStream, cxutil::PrecisionedDouble
#include "cxutil/util/logoutput.h"
#include "cxutil/util/Date.h"
//#include"buildengineinfo.h"
...
, layer_nr(0)
, total_gen_gcode_layers(0)
, relative_extrusion(false)
, use_skirt(false)
, is_firstlayerSkirt(false)
, count(0)
{
...
void GCodeExport::preSetup(const size_t start_extruder)
{
    ...
    EPlatformAdhesion use_skirtS = scene->settings.get<EPlatformAdhesion>("adhesion_type");
    if (use_skirtS != EPlatformAdhesion::NONE)
        use_skirt = true;
    ...
    for (size_t extruder_nr = 0; extruder_nr < extruder_count; extruder_nr++)
    {
        ...
        setFilamentDiameter(extruder_nr, extruder.settings->get<coord_t>("material_diameter"));

        extruder_attr[extruder_nr].last_retraction_prime_speed = extruder.settings->get<Velocity>("retraction_prime_speed"); // the alternative would be switch_extruder_prime_speed, but dual extrusion might not even be configured...
        extruder_attr[extruder_nr].fan_number = extruder.settings->get<size_t>("machine_extruder_cooling_fan_number");

		//init 
		extruder_attr[extruder_nr].retraction_e_amount_current = 0;
		extruder_attr[extruder_nr].retraction_e_amount_at_e_start = 0;
    }
    ...
    TimeEstimateParam param;
    param.machine_max_feedrate_x = mesh_group->settings->get<Velocity>("machine_max_feedrate_x");
    param.machine_max_feedrate_y = mesh_group->settings->get<Velocity>("machine_max_feedrate_y");
    param.machine_max_feedrate_z = mesh_group->settings->get<Velocity>("machine_max_feedrate_z");
    param.machine_max_feedrate_e = mesh_group->settings->get<Velocity>("machine_max_feedrate_e");
    param.machine_max_acceleration_x = mesh_group->settings->get<Acceleration>("machine_max_acceleration_x");
    param.machine_max_acceleration_y = mesh_group->settings->get<Acceleration>("machine_max_acceleration_y");
    param.machine_max_acceleration_z = mesh_group->settings->get<Acceleration>("machine_max_acceleration_z");
    param.machine_max_acceleration_e = mesh_group->settings->get<Acceleration>("machine_max_acceleration_e");
    param.machine_acceleration = mesh_group->settings->get<Velocity>("machine_acceleration");
    param.machine_max_jerk_xy = mesh_group->settings->get<Velocity>("machine_max_jerk_xy");
    param.machine_max_jerk_z = mesh_group->settings->get<Velocity>("machine_max_jerk_z");
    param.machine_max_jerk_e = mesh_group->settings->get<Velocity>("machine_max_jerk_e");
    param.machine_minimum_feedrate = mesh_group->settings->get<Velocity>("machine_minimum_feedrate");

    param.relative_extrusion = mesh_group->settings->get<bool>("relative_extrusion");
    param.machine_use_extruder_offset_to_offset_coords = mesh_group->settings->get<bool>("machine_use_extruder_offset_to_offset_coords");;
    estimateCalculator.setFirmwareDefaults(param);
}
...
void GCodeExport::setInitialAndBuildVolumeTemps(const unsigned int start_extruder_nr)
{
    ...
    for (size_t extruder_nr = 0; extruder_nr < extruder_count; extruder_nr++)
    {
        ...
        const Temperature temp = (extruder_nr == start_extruder_nr)? print_temp_here : extruder.settings->get<Temperature>("material_standby_temperature");
...
{
    //writeComment("Creality Print GCode Generated by CXEngine:" MAGE_VERSION_GIT_HEAD_BRANCH);
...
void GCodeExport::writeMashineConfig()
{
    ...
    tmp << ";ExtruderParams[0] Nozzle Diameter" << scene->extruders.at(0).settings->get<coord_t>("machine_nozzle_tip_outer_diameter") << new_line;
    if (exsize > 1)  ///
    {
        for (int i = 1; i < exsize; i++)
        {
            tmp << ";ExtruderParams[" << i << "] Nozzle Diameter" << scene->extruders.at(i).settings->get<coord_t>("machine_nozzle_tip_outer_diameter") << new_line;
        }
    }
...
std::string GCodeExport::getFileHeader(const std::vector<bool>& extruder_is_used, const Duration* print_time, const std::vector<double>& filament_used, const std::vector<std::string>& mat_ids)
{
    ...
    case EGCodeFlavor::GRIFFIN:
        ...
        prefix << ";PRINT.GROUPS:" << scene->mesh_groups.size() << new_line;
    ...
    default:
        ...
        if (scene->callback)
        {
            scene->callback->onPrintTime((print_time) ? static_cast<int>(*print_time) : 6666);
        }
        if (flavor == EGCodeFlavor::ULTIGCODE)
        {
            ...
            prefix << ";NOZZLE_DIAMETER:" << scene->extruders[0].settings->get<double>("machine_nozzle_size") << new_line;
        }
        else if (flavor == EGCodeFlavor::REPRAP || flavor == EGCodeFlavor::MARLIN || flavor == EGCodeFlavor::MARLIN_VOLUMATRIC)
        {
            ...
            if (filament_used.size() > 0)
            {
                for (unsigned i = 0; i < filament_used.size(); ++i)
                {
                    ...
                    if (flavor != EGCodeFlavor::MARLIN_VOLUMATRIC)
                    {
                        ...
                        if (scene->callback)
                        {
                            scene->callback->onFilamentLen(filament_used[0] / (1000 * extruder_attr[0].filament_area));
                        }
                    }
            ...
            prefix << new_line;
            prefix << ";Layer height: " << scene->current_mesh_group->settings->get<double>("layer_height") << new_line;
        }
...
void GCodeExport::setLayerNr(unsigned int layer_nr_) {
    layer_nr = layer_nr_;
    if (scene->sCallback)
        scene->sCallback->onLayerStart((int)layer_nr, 0.0f);
}

unsigned int GCodeExport::getLayerNr() {
    return layer_nr;
}
...
void GCodeExport::writeOutputStream(const std::string dataString)
{
    output_stream->write(dataString.data(), strlen(dataString.data()));
    previewImageDataEnd = output_stream->tellp();
}
...
Point GCodeExport::getGcodePos(const coord_t x, const coord_t y, const int extruder_train) const
{
    ...
    {
        const Settings& extruder_settings = scene->extruders[extruder_train].settings;
...
void GCodeExport::setFlavor(EGCodeFlavor flavor)
{
	...
	else if (flavor == EGCodeFlavor::MACH3_CREALITY)
	{
		for (int n = 0; n < MAX_EXTRUDERS; n++)
		{
			extruder_attr[n].extruderCharacter = 'P';
		}
	}
    else
    {
...
double GCodeExport::getFilamentDiameter()
{
    return extruder_attr[current_extruder].filament_area;
}

double GCodeExport::getCurrentExtrudedVolume() const
{
    double extrusion_amount = current_e_value;
    const Settings& extruder_settings = scene->extruders[current_extruder].settings;
...
{
    output_stream->seekp(previewImageDataEnd, std::ios::beg);
...
void GCodeExport::writeG12TopHeater(const Temperature& temperature)
{
	if (temperature.value >= 0)
	{
		*output_stream << "T1" << new_line;
		*output_stream << "M109";
		*output_stream << " S" << PrecisionedDouble{ 1, temperature } << new_line;
		*output_stream << "T0" << new_line;
	}
}
...
void GCodeExport::writeTypeComment(const PrintFeatureType& type)
{
    switch (type)
    {
        ...
        case PrintFeatureType::SupportInfill:
            *output_stream << ";TYPE:SUPPORT-INFILL" << new_line;
            break;
        ...
        case PrintFeatureType::PrimeTower:
            *output_stream << ";TYPE:PRIME-TOWER" << new_line;
            break;
		case PrintFeatureType::SlowFlowTypes:
			*output_stream << ";TYPE:Slow-Flow-Types" << new_line;
			break;
		case PrintFeatureType::FlowAdanceTypes:
			*output_stream << ";TYPE:Flow-In-Advance-Types" << new_line;
			break;
...
void GCodeExport::writeExtrusionMode(bool set_relative_extrusion_mode)
{
	if (set_relative_extrusion_mode)
	{
		...
		{
			*output_stream << "G91 ;machine:G25:relative extrusion mode" << new_line;
		}
		else
		{
	...
	else
	{
		if (flavor == EGCodeFlavor::MACH3_CREALITY)
		{
			*output_stream << "G90 ;machine:G25:relative extrusion mode" << new_line;
		}
		else
		{
...
void GCodeExport::writeTravel(const Point& p, const Velocity& speed)
{
    ...
    {
        Point3 offset = scene->mesh_groups.begin()->m_offset;
        Point3 axis = Point3(0, 1, 0);
        if (Axis == "X") axis = Point3(1, 0, 0);
        Point3 input_pt = RotateByVector(Point3(p.X, p.Y, current_layer_z), axis, offset, -angle);
...
void GCodeExport::writeExtrusion(const Point& p, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature, bool update_extrusion_offset)
{
    ...
    if (angle != 0. && layer_nr >= 0)
    {
        ...
        if (z_offset > 0)
        {
            ...
            double s = sin(angle * M_PIf / 180);
            double c = cos(angle * M_PIf / 180);
...
void GCodeExport::writeMoveBFB(const int x, const int y, const int z, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature)
{
    ...
    {
        LOGE(scene->m_sliceLogSortId,"Extrusion rate is infinite!");
        assert(false && "Infinite extrusion move!");
        std::exit(1);
    }
    if (std::isnan(extrusion_mm3_per_mm))
    {
        LOGE(scene->m_sliceLogSortId, "Extrusion rate is not a number!");
    ...
    if (rpm > 0)
    {
        if (extruder_attr[current_extruder].retraction_e_amount_current)
        {
            if (currentSpeed != double(rpm))
            {
                ...
                *output_stream << "M108 S" << cxutil::PrecisionedDouble{1, rpm} << new_line;
    ...
    *output_stream << "G1 X" << cxutil::MMtoStream{gcode_pos.X} << " Y" << cxutil::MMtoStream{gcode_pos.Y} << " Z" << cxutil::MMtoStream{z};
    *output_stream << " F" << cxutil::PrecisionedDouble{1, fspeed} << new_line;
...
void GCodeExport::writeTravel(const coord_t& x, const coord_t& y, const coord_t& z, const Velocity& speed)
{
    ...
    const double layer_height = scene->current_mesh_group->settings->get<double>("layer_height");
    scene->communication->sendLineTo(travel_move_type, Point(x, y), display_width, layer_height, speed);
...
void GCodeExport::writeFXYZE(const Velocity& speed, const int x, const int y, const int z, const double e, const PrintFeatureType& feature)
{
    ...
    {
        *output_stream << " F" << cxutil::PrecisionedDouble{1, speed * 60};
        currentSpeed = speed;
    }
	...
	int new_Y = gcode_pos.Y;
	*output_stream << " X" << cxutil::MMtoStream{ gcode_pos.X } << " Y" << cxutil::MMtoStream{ new_Y };
	if (z != currentPosition.z)
	{
		int new_Z = z;
		*output_stream << " Z" << cxutil::MMtoStream{ new_Z };
	}
    ...
    if (e + current_e_offset != current_e_value)
    {
        ...
        *output_stream << " " << extruder_attr[current_extruder].extruderCharacter << cxutil::PrecisionedDouble{5, output_e};
    }
    *output_stream << new_line;
	...
	if (scene->sCallback)
		scene->sCallback->onWrite(x, y, z, speed, feature);
}
...
void GCodeExport::writeUnretractionAndPrime()
{
        ...
        { // note that BFB is handled differently
            *output_stream << "G11" << new_line;
            ...
            if (prime_volume != 0)
            {
                ...
                *output_stream << "G1 F" << cxutil::PrecisionedDouble{1, extruder_attr[current_extruder].last_retraction_prime_speed * 60} << " " << extruder_attr[current_extruder].extruderCharacter << cxutil::PrecisionedDouble{5, output_e} << new_line;
    ...
    {
        const double output_e = (relative_extrusion)? prime_volume_e : current_e_value;
        *output_stream << "G1 F" << cxutil::PrecisionedDouble{1, extruder_attr[current_extruder].last_retraction_prime_speed * 60} << " " << extruder_attr[current_extruder].extruderCharacter;
        *output_stream << cxutil::PrecisionedDouble{5, output_e} << new_line;
        currentSpeed = extruder_attr[current_extruder].last_retraction_prime_speed;
	...
	if (flavor == EGCodeFlavor::MACH3_CREALITY && getCurrentExtrudedVolume() > 900000.0)
	{
		resetExtrusionValue();
	}
    else if (getCurrentExtrudedVolume() > 10000.0 && flavor != EGCodeFlavor::BFB && flavor != EGCodeFlavor::MAKERBOT) //According to https://github.com/Ultimaker/CuraEngine/issues/14 having more then 21m of extrusion causes inaccuracies. So reset it every 10m, just to be sure.
    {
        resetExtrusionValue();
    }
    if (extruder_attr[current_extruder].retraction_e_amount_current)
...
void GCodeExport::writeRetraction(const RetractionConfig& config, bool force, bool extruder_switch)
{
    ...
    else
    {
        ...
        *output_stream << "G1 F" << cxutil::PrecisionedDouble{1, speed} << " " << extr_attr.extruderCharacter << cxutil::PrecisionedDouble{5, output_e} << new_line;
        currentSpeed = speed;
        estimateCalculator.plan(TimeEstimateCalculator::Position(INT2MM(currentPosition.x), INT2MM(currentPosition.y), INT2MM(currentPosition.z), eToMm(current_e_value)), currentSpeed, PrintFeatureType::MoveRetraction);
...
void GCodeExport::writeZhopStart(const coord_t hop_height, Velocity speed/*= 0*/)
{
    if (hop_height > 0)
    {
        if (speed == 0)
        {
            const ExtruderTrain& extruder = scene->extruders[current_extruder];
            speed = extruder.settings->get<Velocity>("speed_z_hop");
        }
        is_z_hopped = hop_height;
        currentSpeed = speed;
        *output_stream << "G1 F" << cxutil::PrecisionedDouble{1, speed * 60} << " Z" << cxutil::MMtoStream{current_layer_z + is_z_hopped} << new_line;
...
void GCodeExport::writeZhopEnd(Velocity speed/*= 0*/)
{
    if (is_z_hopped)
    {
        ...
        currentSpeed = speed;
        *output_stream << "G1 F" << cxutil::PrecisionedDouble{1, speed * 60} << " Z" << cxutil::MMtoStream{current_layer_z} << new_line;
        assert(speed > 0.0 && "Z hop speed should be positive.");
    }
}
...
void GCodeExport::startExtruder(const size_t new_extruder)
{
    ...
    if (scene->sCallback)
        scene->sCallback->onExtruderChanged((int)current_extruder);

    assert(getCurrentExtrudedVolume() == 0.0 && "Just after an extruder switch we haven't extruded anything yet!");
    resetExtrusionValue(); // zero the E value on the new extruder, just to be sure

    const std::string start_code = scene->extruders[new_extruder].settings->get<std::string>("machine_extruder_start_code");
    ...
    scene->communication->setExtruderForSend(scene->extruders[new_extruder]);
    scene->communication->sendCurrentPosition(getPositionXY());
...
void GCodeExport::switchExtruder(size_t new_extruder, const RetractionConfig& retraction_config_old_extruder, coord_t perform_z_hop /*= 0*/)
{
    ...
    const Settings& old_extruder_settings = scene->extruders[current_extruder].settings;
    ...
    if (flavor == EGCodeFlavor::GRIFFIN)
    {
        ...
        *output_stream << command << new_line;
        ...
        if (should_correct_z)
        {
            ...
            *output_stream << "G0 Z" << cxutil::MMtoStream{getPositionZ()} << new_line;
        }
    }
    else
    {
...
void GCodeExport::writeFanCommand(double speed)
{
    ...
    {
        *output_stream << "M106 S" << cxutil::PrecisionedDouble{1, speed * 255 / 100};
        if (fan_number)
        {
...
{
    const ExtruderTrain& extruder_extruder = scene->extruders[extruder];
    //bed tmp wait liuhong 2022-04-25 
    bool bedtmpwait = extruder_extruder.settings->get<bool>("material_bed_temp_wait"); 

    if (!extruder_extruder.settings->get<bool>("machine_nozzle_temp_enabled"))
    {
        return;
    }

    if (extruder_extruder.settings->get<bool>("machine_extruders_share_heater"))
    {
...
#endif // ASSERT_INSANE_OUTPUT
    *output_stream << " S" << cxutil::PrecisionedDouble{1, temperature} << new_line;
    bool waitsync = extruder_extruder.settings->get<bool>("bed_print_temp_wait_sync"); 
    
    if (bedtmpwait && wait && (flavor == EGCodeFlavor::MARLIN) && waitsync && scene->vctHotZone.empty())
    {
        cxutil::Temperature bedtmp = extruder_extruder.settings->get<Temperature>("material_bed_temperature");
        *output_stream << "M105" << new_line;
        *output_stream << "M190 S";
        *output_stream << cxutil::PrecisionedDouble{ 1, bedtmp } << new_line;
    }
    //wait bet tmp after print tmp liuhong add 2022-04-25
    if (wait && flavor == EGCodeFlavor::MAKERBOT)
    {
...
void GCodeExport::writeBedTemperatureCommand(const Temperature& temperature, std::vector<bool>& vctHotZone, const bool wait)
{
    if (flavor == EGCodeFlavor::ULTIGCODE)
    ...
    if (wait)
    {
        if(flavor == EGCodeFlavor::MARLIN)
        {
            if (vctHotZone.size())
            {
				for (int n = 0; n < vctHotZone.size(); n++)
				{
					if (vctHotZone[n])
					{
						*output_stream << "M140 D";
						*output_stream << n << " S";
						*output_stream << cxutil::PrecisionedDouble{ 1, temperature } << new_line;
					}
				}
                *output_stream << "M105" << new_line;
				for (int n = 0; n < vctHotZone.size(); n++)
            ...
            else
            {
				...
				*output_stream << "M105" << new_line;
				...
				//set wait after print_temp_wait or wait temp sync liuhong 2022-04-25
                const ExtruderTrain& extruder_extruder = scene->extruders[0];
                bool waitsync = extruder_extruder.settings->get<bool>("bed_print_temp_wait_sync");
                if (!waitsync)
                {
                    *output_stream << "M190 S";
...
void GCodeExport::writePrintAcceleration(const Acceleration& acceleration)
{
    switch (getFlavor())
    {
        case EGCodeFlavor::REPETIER:
            ...
            {
                *output_stream << "M201 X" << cxutil::PrecisionedDouble{0, acceleration} << " Y" << cxutil::PrecisionedDouble{0, acceleration} << new_line;
            }
            break;
        case EGCodeFlavor::REPRAP:
            if (current_print_acceleration != acceleration)
            {
                *output_stream << "M204 P" << cxutil::PrecisionedDouble{0, acceleration} << new_line;
            }
            break;
        default:
            ...
            if (current_print_acceleration != acceleration)
            {
                *output_stream << "M204 S" << cxutil::PrecisionedDouble{0, acceleration} << new_line;
            }
            break;
    }
...
void GCodeExport::writeTravelAcceleration(const Acceleration& acceleration)
{
    switch (getFlavor())
    {
        case EGCodeFlavor::REPETIER:
            ...
            {
                *output_stream << "M202 X" << cxutil::PrecisionedDouble{0, acceleration} << " Y" << cxutil::PrecisionedDouble{0, acceleration} << new_line;
            }
            break;
        case EGCodeFlavor::REPRAP:
            if (current_travel_acceleration != acceleration)
            {
                *output_stream << "M204 T" << cxutil::PrecisionedDouble{0, acceleration} << new_line;
            }
            break;
        default:
            // MARLIN, etc. only have one acceleration for both print and travel
...
void GCodeExport::writeJerk(const Velocity& jerk)
{
    if (current_jerk != jerk)
    {
        ...
        {
            case EGCodeFlavor::REPETIER:
                *output_stream << "M207 X" << cxutil::PrecisionedDouble{2, jerk} << new_line;
                break;
            case EGCodeFlavor::REPRAP:
                *output_stream << "M566 X" << cxutil::PrecisionedDouble{2, jerk * 60} << " Y" << cxutil::PrecisionedDouble{2, jerk * 60} << new_line;
                break;
            default:
                *output_stream << "M205 X" << cxutil::PrecisionedDouble{2, jerk} << " Y" << cxutil::PrecisionedDouble{2, jerk} << new_line;
                break;
        }
        current_jerk = jerk;
