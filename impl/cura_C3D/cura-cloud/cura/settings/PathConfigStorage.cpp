...
#include "cxutil/settings/Settings.h" // MAX_INFILL_COMBINE
...
std::vector<Ratio> PathConfigStorage::getLineWidthFactorPerExtruder(const LayerIndex& layer_nr)
{
    ...
    for (const ExtruderTrain& extruder : scene->extruders)
    {
        if (layer_nr <= 0)
        {
            const Ratio factor = extruder.settings->get<Ratio>("initial_layer_line_width_factor");
...
: scene(storage.scene)
, support_infill_extruder_nr(scene->current_mesh_group->settings->get<ExtruderTrain&>("support_infill_extruder_nr").extruder_nr)
, support_roof_extruder_nr(scene->current_mesh_group->settings->get<ExtruderTrain&>("support_roof_extruder_nr").extruder_nr)
, support_bottom_extruder_nr(scene->current_mesh_group->settings->get<ExtruderTrain&>("support_bottom_extruder_nr").extruder_nr)
, adhesion_extruder_train(scene->current_mesh_group->settings->get<ExtruderTrain&>("adhesion_extruder_nr"))
, support_infill_train(scene->extruders[support_infill_extruder_nr])
, support_roof_train(scene->extruders[support_roof_extruder_nr])
, support_bottom_train(scene->extruders[support_bottom_extruder_nr])
, line_width_factor_per_extruder(getLineWidthFactorPerExtruder(layer_nr))
, raft_base_config(
...
, support_bottom_config(
            ...
            , layer_thickness
            , support_bottom_train.settings->get<Ratio>("support_bottom_material_flow") * ((layer_nr == 0) ? support_bottom_train.settings->get<Ratio>("material_flow_layer_0") : Ratio(1.0))
	...
	, slow_flow_travel_config(
		PrintFeatureType::SlowFlowTypes
		, adhesion_extruder_train.settings->get<coord_t>("wall_line_width_0")
		, layer_thickness
		, adhesion_extruder_train.settings->get<Ratio>("special_material_flow_travel")
		, GCodePathConfig::SpeedDerivatives{ scene->extruders[0].settings->get<Velocity>("speed_travel"), scene->extruders[0].settings->get<Acceleration>("acceleration_travel"), scene->extruders[0].settings->get<Velocity>("jerk_travel") }
	)
	, flow_in_advance_config(
		PrintFeatureType::FlowAdanceTypes
		, adhesion_extruder_train.settings->get<coord_t>("wall_line_width_0")
		, layer_thickness
		, adhesion_extruder_train.settings->get<Ratio>("special_advance_flow")
        , GCodePathConfig::SpeedDerivatives{ scene->extruders[0].settings->get<Velocity>("speed_travel"), scene->extruders[0].settings->get<Acceleration>("acceleration_travel"), scene->extruders[0].settings->get<Velocity>("jerk_travel") }
	)
{
    ...
    for (size_t extruder_nr = 0; extruder_nr < extruder_count; extruder_nr++)
    {
        ...
        travel_config_per_extruder.emplace_back(
                ...
                , 0
                , 0.0
                , GCodePathConfig::SpeedDerivatives{extruder.settings->get<Velocity>("speed_travel"), extruder.settings->get<Acceleration>("acceleration_travel"), extruder.settings->get<Velocity>("jerk_travel")}
            );
        skirt_brim_config_per_extruder.emplace_back(
                PrintFeatureType::SkirtBrim
                , extruder.settings->get<coord_t>("skirt_brim_line_width")
                ...
                , layer_thickness
                , extruder.settings->get<Ratio>("skirt_brim_material_flow") * ((layer_nr == 0) ? extruder.settings->get<Ratio>("material_flow_layer_0") : Ratio(1.0))
                , GCodePathConfig::SpeedDerivatives{extruder.settings->get<Velocity>("skirt_brim_speed"), extruder.settings->get<Acceleration>("acceleration_skirt_brim"), extruder.settings->get<Velocity>("jerk_skirt_brim")}
            );
        prime_tower_config_per_extruder.emplace_back(
                PrintFeatureType::PrimeTower
                , extruder.settings->get<coord_t>("prime_tower_line_width")
                ...
                , layer_thickness
                , extruder.settings->get<Ratio>("prime_tower_flow") * ((layer_nr == 0) ? extruder.settings->get<Ratio>("material_flow_layer_0") : Ratio(1.0))
                , GCodePathConfig::SpeedDerivatives{extruder.settings->get<Velocity>("speed_prime_tower"), extruder.settings->get<Acceleration>("acceleration_prime_tower"), extruder.settings->get<Velocity>("jerk_prime_tower")}
            );
    }
...
void PathConfigStorage::handleInitialLayerSpeedup(const SliceDataStorage& storage, const LayerIndex& layer_nr, const size_t initial_speedup_layer_count)
{
    ...
    global_first_layer_config_per_extruder.reserve(storage.scene->extruders.size());
    for (const ExtruderTrain& extruder : storage.scene->extruders)
    {
    ...
    { // extruder configs: travel, skirt/brim (= shield)
        ...
        {
            const ExtruderTrain& extruder = storage.scene->extruders[extruder_nr];
            GCodePathConfig::SpeedDerivatives initial_layer_travel_speed_config{
                    extruder.settings->get<Velocity>("speed_travel_layer_0")
                    , extruder.settings->get<Acceleration>("acceleration_travel_layer_0")
                    , extruder.settings->get<Velocity>("jerk_travel_layer_0")
            };
