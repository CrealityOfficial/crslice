...
void WallsComputation::generateInsets(SliceLayerPart* part)
{
    ...
    const bool recompute_outline_based_on_outer_wall = (settings.get<bool>("support_enable") || 
        settings.get<bool>("support_tree_enable")) && !settings.get<bool>("fill_outline_gaps");  
    size_t inset_count_hole = settings.get<size_t>("hole_wall_line_count");
    bool exist_thin_wall = false;

    auto relation = [&](Polygons newpoly, Polygons oldpoly)
    {
        if (newpoly.size() != oldpoly.size())
            return false;
        for (int i = 0; i < newpoly.size(); i++)
        {
            if (ClipperLib::Orientation(newpoly.paths[i]) != ClipperLib::Orientation(oldpoly.paths[i]))
                return false;
        }
        return true;
    };
    ...
    for(size_t i = 0; i < inset_count; i++)
    {
        ...
        part->insets_negtive.push_back(Polygons());
        if (i == 0)
        {
        ...
        else if (i == 1)
        {
            ...
            if (!exist_thin_wall) exist_thin_wall = !relation(part->insets[1], part->insets[0]);
        }
        else
        {
            part->insets[i] = part->insets[i - 1].offset(-line_width_x);
            if (!exist_thin_wall) exist_thin_wall = !relation(part->insets[i], part->insets[i - 1]);
        }

        if (!exist_thin_wall && inset_count > inset_count_hole)
        {
            Polygons polygon_p, polygon_n;
            for (ClipperLib::Path path : part->insets[i])
            {
            ...
            part->insets_negtive[i] = polygon_n;
            if(i >= inset_count_hole)
                part->insets[i].paths.swap(polygon_p.paths);
        }
