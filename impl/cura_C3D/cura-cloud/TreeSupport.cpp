...
#include "cxutil/settings/AngleRadians.h" //Creating the correct branch angles.
#include "cxutil/settings/Ratio.h"
#include "cxutil/math/IntPoint.h" //To normalize vectors.
#include "cxutil/util/MinimumSpanningTree.h" //For connecting the correct nodes together to form an efficient tree.
#include "cxutil/math/polygon.h" //For splitting polygons into parts.
#include "cxutil/util/polygonUtils.h" //For moveInside.
...
{
    const bool global_use_tree_support = storage.scene->current_mesh_group->settings->get<bool>("support_tree_enable");
...
void TreeSupport::drawCircles(SliceDataStorage& storage, const std::vector<std::unordered_set<Node*>>& contact_nodes)
{
    ...
    cxutil::Polygon branch_circle; //Pre-generate a circle with correct diameter so that we don't have to recompute those (co)sines every time.
...
#pragma omp parallel for shared(storage, contact_nodes)
    ...
    for (int layer_nr = 0; layer_nr < static_cast<int>(contact_nodes.size()); layer_nr++)
    {
        ...
        storage.support.supportLayers[layer_nr].support_infill_parts.clear();//È¥µôÍâ²ã¿ò£¬½öÁôÌî³ä
...
#pragma omp critical (progress)
        {
            ...
            storage.scene->progress.messageProgress(Progress::Stage::SUPPORT, progress_contact_nodes + progress_current, progress_contact_nodes + progress_total);
        }
    }
}

void TreeSupport::dropNodes(SliceDataStorage& storage, std::vector<std::unordered_set<Node*>>& contact_nodes)
{
    const Settings& mesh_group_settings = *storage.scene->current_mesh_group->settings;
    ...
    for (size_t layer_nr = contact_nodes.size() - 1; layer_nr > 0; layer_nr--) //Skip layer 0, since we can't drop down the vertices there.
    {
        ...
        storage.scene->progress.messageProgress(Progress::Stage::SUPPORT, progress_current, progress_total);
    }
...
Polygons ModelVolumes::calculateMachineBorderCollision(cxutil::Polygon machine_border)
{
