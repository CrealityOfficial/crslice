...
Ratio LayerPlan::getLatestPathSpeedFactor()
{
    std::vector<GCodePath>& paths = extruder_plans.back().paths;
    if (paths.empty() || paths.back().isTravelPath())
        return Ratio(1.0);
    else
        return paths.back().speed_factor;    
}
...
, layer_nr(layer_nr)
, is_initial_layer(layer_nr == 0 - static_cast<LayerIndex>(Raft::getTotalExtraLayers(storage.scene)))
, is_raft_layer(layer_nr < 0 - static_cast<LayerIndex>(Raft::getFillerLayerCount(storage.scene)))
, layer_thickness(layer_thickness)
, has_prime_tower_planned_per_extruder(scene->extruders.size(), false)
, current_mesh("NONMESH")
, last_extruder_previous_layer(start_extruder)
, last_planned_extruder(&scene->extruders[start_extruder])
...
, fan_speed_layer_time_settings_per_extruder(fan_speed_layer_time_settings_per_extruder)
{
    ...
    if (scene->current_mesh_group->settings->get<CombingMode>("retraction_combing") != CombingMode::OFF)
    {
    ...
    extruder_plans.reserve(scene->extruders.size());
    ...
    //std::cout << "construct " << layer_nr << std::endl;
}
...
LayerPlan::~LayerPlan()
{
    ...
    //std::cout << "destruct " << layer_nr << std::endl;
}
...
{
    const CombingMode combing_mode = scene->current_mesh_group->settings->get<CombingMode>("retraction_combing");
...
bool LayerPlan::setExtruder(const size_t extruder_nr)
{
    ...
    else 
    {
        ...
        assert(extruder_plans.size() <= scene->extruders.size() && "Never use the same extruder twice on one layer!");
    }
    last_planned_extruder = &scene->extruders[extruder_nr];
...
cxutil::optional<std::pair<Point, bool>> LayerPlan::getFirstTravelDestinationState() const
{
    cxutil::optional<std::pair<Point, bool>> ret;
...
GCodePath& LayerPlan::addTravel(const Point p, const bool force_retract)
{
    ...
    if (retraction_config.travelFlowEnable && last_planned_position && 
        shorterThen(*last_planned_position - p, retraction_config.travelFlow_distance))
    {
		path->retract = false;
		path->perform_z_hop = false;
		path->bslowFlow = retraction_config.travelFlowEnable;
		path->config = &configs_storage.slow_flow_travel_config;
    }
    else if (!is_first_travel_of_layer && last_planned_position && 
        shorterThen(*last_planned_position - p, retraction_config.retraction_min_travel_distance))
    {
        path->retract = false;
        path->perform_z_hop = false;
    }
	else if (retraction_config.advanceEnable)
	{
		path->length_flow = retraction_config.length_flow_advance;
		path->config = &configs_storage.flow_in_advance_config;
		//path->retract = false;
	}
...
void LayerPlan::addWallLine(const Point& p0, const Point& p1, const SliceMeshStorage& mesh, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, float flow, float& non_bridge_line_volume, Ratio speed_factor, double distance_to_bridge_start)
{
    ...
    const bool set_wall_overhang_grading = mesh.settings->get<bool>("set_wall_overhang_grading");
    std::vector<Ratio> overhang_speed_factor_vec;
    if (set_wall_overhang_grading)
    {
        for (int i = 1; i < 5; i++)
        {
            overhang_speed_factor_vec.push_back(Ratio(mesh.settings->get<Velocity>("wall_overhang_speed_" + std::to_string(i)) / non_bridge_config.getSpeed()));
        }
    }
    else
    {
    ...
    auto getSpeedFactor = [&](const Point& p0, const Point& p1)
    {
        ...
        bool small_feature = vSize2(p0 - p1) < 25000000;
        ...
        if (sf0 < speed_factor && sf1 < speed_factor || ((sf0 < speed_factor  || sf1 < speed_factor) && small_feature))
            return std::min(sf0, sf1);
        else
            return speed_factor;
    };

    Ratio sub_speed_factor = getSpeedFactor(p0, p1);
    if (sub_speed_factor < 0.5 * getLatestPathSpeedFactor())
    {
        addTravel(p0 + Point(800,800), true);
        addTravel(p0 + Point(800, 0));
        addTravel(p0);
    }
	
	
	double fan_speed = -1;
    if (sub_speed_factor < speed_factor)
    {
        fan_speed = bridge_config.getFanSpeed();
    }
        ...
        while (distance_to_line_end > min_line_len)
        {
            ...
            if (distance_to_bridge_start > 0)
            {
                ...
                else
                {
                    ...
                    addExtrusionMove(segment_end, non_bridge_config, SpaceFillType::Polygons, segment_flow, spiralize, sub_speed_factor, fan_speed);
                }
    ...
    {
        // no bridges required
        addExtrusionMove(p1, non_bridge_config, SpaceFillType::Polygons, flow, spiralize, sub_speed_factor, fan_speed);
    }
    else
    {
...
void LayerPlan::addWall(ConstPolygonRef wall, int start_idx, const SliceMeshStorage& mesh, const GCodePathConfig& non_bridge_config, const GCodePathConfig& bridge_config, WallOverlapComputation* wall_overlap_computation, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract)
{
    ...
    if (wall.area() < 0) //hole walls
        speed_factor = mesh.settings->get<Ratio>("wall_hole_speed_factor");
    ...
    bool is_small_feature = (small_feature_max_length > 0) && wall.shorterThan(small_feature_max_length);
    ...
    int64_t wall_length = wall.polygonLength();
    if (mesh.settings->get<bool>("set_small_feature_grading"))
    {
        int level = -1;
        if (wall_length < mesh.settings->get<coord_t>("special_small_feature_max_length_4"))
        {
            level = 4;
            small_feature_speed_factor = mesh.settings->get<Ratio>((layer_nr == 0) ? "special_small_feature_speed_factor_0_4" : "special_small_feature_speed_factor_4");
            extruder_plans.back().cool_min_layer_time_correct = mesh.settings->get<Duration>("special_cool_min_layer_time_4");
            is_small_feature = true;
        }
        else if (wall_length < mesh.settings->get<coord_t>("special_small_feature_max_length_3"))
        {
            level = 3;
            small_feature_speed_factor = mesh.settings->get<Ratio>((layer_nr == 0) ? "special_small_feature_speed_factor_0_3" : "special_small_feature_speed_factor_3");
            extruder_plans.back().cool_min_layer_time_correct = mesh.settings->get<Duration>("special_cool_min_layer_time_3");
            is_small_feature = true;
        }
        else if (wall_length < mesh.settings->get<coord_t>("special_small_feature_max_length_2"))
        {
            level = 2;
            small_feature_speed_factor = mesh.settings->get<Ratio>((layer_nr == 0) ? "special_small_feature_speed_factor_0_2" : "special_small_feature_speed_factor_2");
            extruder_plans.back().cool_min_layer_time_correct = mesh.settings->get<Duration>("special_cool_min_layer_time_2");
            is_small_feature = true;
        }
        else if (wall_length < mesh.settings->get<coord_t>("special_small_feature_max_length_1"))
        {
            level = 1;
            small_feature_speed_factor = mesh.settings->get<Ratio>((layer_nr == 0) ? "special_small_feature_speed_factor_0_1" : "special_small_feature_speed_factor_1");
            extruder_plans.back().cool_min_layer_time_correct = mesh.settings->get<Duration>("special_cool_min_layer_time_1");
            is_small_feature = true;
        }
        if (level > 0 && level < 4)
        {
    ...
    auto computeDistanceToBridgeStart = [&](unsigned current_index)
    {
        ...
        if (!bridge_wall_mask.empty())
        {
            ...
            for (int point_idx = current_index; point_idx < wall.size(); ++point_idx)
            {
...
void LayerPlan::addLinesByOptimizer(const Polygons& polygons, const GCodePathConfig& config, SpaceFillType space_fill_type, bool enable_travel_optimization, int wipe_dist, float flow_ratio, cxutil::optional<Point> near_start_location, double fan_speed)
{
    auto getStartPoint = [&near_start_location](Polygons raftLines)
    {
        if (raftLines.empty()) return;
        int dirationIdx = -1;//infill angle,0->0°, 1->45°, 2->90°, 3->135°
        std::vector<int> dir_num_record(4, 0);
        for (ClipperLib::Path path : raftLines)//get infill angle
        {
            if (path[0] == path[1])
            {
                continue;
            ...
            if ((path[0].X - path[1].X) / (path[0].Y - path[1].Y) == 1.)
            {
                dir_num_record[1] ++;
                if (dir_num_record[1] > 50)
                {
                    dirationIdx = 1;
                    break;
                }
        ...
        {
            int max_num = 0;
            for (int i = 0; i < dir_num_record.size(); i++)
            {
                if (dir_num_record[i] > max_num)
                {
                    dirationIdx = i;
                    max_num = dir_num_record[i];
                }
            }
        }

        Point startPoint = near_start_location.value_or(Point());
        coord_t minX = INT_MAX, maxX = 0, minY = INT_MAX, maxY = 0;
        for (int i = 0; i < raftLines.size(); i++)
        {
            for (Point pt : raftLines[i])
            {
        ...
        Point midPoint = Point((minX + maxX) / 2, (minY + maxY) / 2);
        switch (dirationIdx)
        {
        case 0:
        {
            startPoint.X = startPoint.X > midPoint.X ? 2 * maxX : 0;
            startPoint.Y = midPoint.Y;
            break;
        }
        case 1:
        {
            if (vSize2(startPoint - Point(maxX, minY)) < vSize2(startPoint - Point(minX, maxY)))
                startPoint = Point(maxX, minY);
            else
                startPoint = Point(minX, maxY);
            break;
        }
        case 2:
        {
            startPoint.X = midPoint.X;
            startPoint.Y = startPoint.Y > midPoint.Y ? 2 * maxY : 0;
            break;
        }
        case 3:
        {
            if (vSize2(startPoint - Point(maxX, maxY)) < vSize2(startPoint - Point(minX, minY)))
                startPoint = Point(maxX, maxY);
            else
        ...
        near_start_location = startPoint;
    };

    if (space_fill_type == SpaceFillType::PolyLines || space_fill_type == SpaceFillType::Lines)
    {
        getStartPoint(polygons);
    }
    ...
    for (unsigned int order_idx = 0; order_idx < orderOptimizer.polyOrder.size(); order_idx++)
    {
        ...
        coord_t lineSize2 = vSize2(p1 - p0);
        if(lineSize2 < MINIMUM_SQUARED_LINE_LENGTH)
        {
            continue;
        }
        addTravel(p0);
...
{
    const bool smooth_contours = scene->current_mesh_group->settings->get<bool>("smooth_spiralized_contours");
    const bool slope_slice = scene->current_mesh_group->settings->get<bool>("special_slope_slice_angle") != 0;
...
void ExtruderPlan::processFanSpeedAndMinimalLayerTime(bool force_minimal_layer_time, Point starting_position)
{
    ...
    */
    ...
    if (force_minimal_layer_time && totalLayerTime < cool_min_layer_time)//层时间<最短单层冷却时间,则风扇开到最大
    {
        fan_speed = fan_speed_layer_time_settings.cool_fan_speed_max;
    }
    else if (cool_min_layer_time >= fan_speed_layer_time_settings.cool_min_layer_time_fan_speed_max)//最短单层冷却时间>=设定的打印时间阈值，则最短单层冷却时间的判断不需要了
    {
        //fan_speed = fan_speed_layer_time_settings.cool_fan_speed_min;
    }
    else if (force_minimal_layer_time && totalLayerTime < fan_speed_layer_time_settings.cool_min_layer_time_fan_speed_max)//层时间<设定的打印时间阈值,则
    {
        // when forceMinimalLayerTime didn't change the extrusionSpeedFactor, we adjust the fan speed
        double fan_speed_diff = fan_speed_layer_time_settings.cool_fan_speed_max - fan_speed_layer_time_settings.cool_fan_speed_min;//最大风扇-正常风扇
        double layer_time_diff = fan_speed_layer_time_settings.cool_min_layer_time_fan_speed_max - cool_min_layer_time;//设定的打印时间阈值-最短单层冷却时间
        double fraction_of_slope = (totalLayerTime - cool_min_layer_time) / layer_time_diff;
...
{
    Communication* communication = scene->communication;
    ...
    if (layer_nr == 1 - static_cast<LayerIndex>(Raft::getTotalExtraLayers(scene)) && mesh_group_settings.get<bool>("machine_heated_bed") && mesh_group_settings.get<Temperature>("material_bed_temperature") != 0)
    {
        constexpr bool wait = false;
        gcode.writeBedTemperatureCommand(mesh_group_settings.get<Temperature>("material_bed_temperature"),scene->vctHotZone, wait);
    }
    ...
    for (size_t extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        if (extruder_nr != extruder_plan.extruder_nr)
        {
            ...
            //const ExtruderTrain& prev_extruder_train = scene->extruders[prev_extruder];
        ...
        else if (extruder_plan_idx == 0)
        {
            ...
            else if (layer_nr != 0 && scene->extruders[extruder_nr].settings->get<bool>("retract_at_layer_change"))
            {
        ...
        for (unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            if (path.retract)
            {
				if (path.config->type == PrintFeatureType::FlowAdanceTypes)
				{
                ...
                bool z_hop = !path.perform_z_hop && final_travel_z != z && extruder_plan_idx == (extruder_plans.size() - 1) && path_idx == (paths.size() - 1);
                if (/*!z_hop && */extruder.settings->get<bool>("retraction_during_traveling") && path.config->isTravelPath() && path.config->type != PrintFeatureType::FlowAdanceTypes)
                {
                    Point current_position = gcode.getPositionXY();
                    bool z_hop = !path.perform_z_hop && final_travel_z != z && extruder_plan_idx == (extruder_plans.size() - 1) && path_idx == (paths.size() - 1);
                    if (z_hop)
                    {
                        gcode.setZ(final_travel_z);
                        gcode.writeExtrusionG1(extruder.settings->get<Velocity>("speed_z_hop"), current_position, -retraction_config.distance, path.config->type);
                    }
                    double speed = path.config->getSpeed() * path.speed_factor;
                    double travel_dis_retract = retraction_config.distance / retraction_config.speed * speed;
                    double travel_dis_retract_prime = retraction_config.distance / retraction_config.primeSpeed * speed;
                    int add_idx = -1, add_idx_prime = -1;
                    double cur_dis = 0, path_dis = 0;
                    if (!z_hop)
                    {
                        std::vector<Point> paths_dst;
                        for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                        {
                            Point cur_point = Point();
                            if (point_idx == 0)
                                cur_point = current_position;
                            else
                                cur_point =path.points[point_idx - 1];
                            cur_dis += vSizeMM(path.points[point_idx] - cur_point);
                            if (cur_dis >= travel_dis_retract && add_idx == -1)
                            {
                                double theta = (travel_dis_retract - path_dis) / vSizeMM(path.points[point_idx] - cur_point);
                                Point theta_pt = path.points[point_idx] - cur_point;
                                Point travel_start = cur_point + Point(theta_pt.X * theta + 0.5, theta_pt.Y * theta + 0.5);
                                paths_dst.push_back(travel_start); 
                                add_idx = point_idx;
                            }
                            paths_dst.push_back(path.points[point_idx]);
                            path_dis = cur_dis;
                        }
                        if(add_idx >= 0)
                            path.points.swap(paths_dst);
                    }

                    cur_dis = 0,path_dis = 0;
                    std::vector<Point> paths_dst;
                    for (int point_idx = path.points.size() - 1; point_idx >= 0; point_idx--)
                    {
                        Point cur_point = Point();
                        if (point_idx - 1 < 0)
                        {
                            ...
                            cur_point = path.points[point_idx - 1];
                        cur_dis += vSizeMM(cur_point - path.points[point_idx]);
                        paths_dst.push_back(path.points[point_idx]);
                        if (cur_dis >= travel_dis_retract_prime && add_idx_prime == -1)
                        {
                            double theta = (travel_dis_retract_prime - path_dis) / vSizeMM(cur_point - path.points[point_idx]);
                            Point theta_pt = cur_point - path.points[point_idx];
                            Point travel_start = path.points[point_idx] + Point(theta_pt.X * theta + 0.5, theta_pt.Y * theta + 0.5);
                            paths_dst.push_back(travel_start);
                            add_idx_prime = point_idx;
                        }                    
                        path_dis = cur_dis;
                    }
                    if (add_idx_prime >= 0)
                    {
                        std::reverse(paths_dst.begin(), paths_dst.end());
                        path.points.swap(paths_dst);
                    }

                    if ((z_hop && add_idx_prime >= 0) || (add_idx_prime > add_idx && add_idx >= 0))
                    {
                        for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                        {
                            double cur_path_len = point_idx == 0 ? vSizeMM(path.points[point_idx] - current_position) : vSizeMM(path.points[point_idx] - path.points[point_idx - 1]);

                            if (!z_hop && point_idx <= add_idx)
                            {
                                double theta = 1.0f;
                                if(add_idx != 0) theta = cur_path_len / travel_dis_retract;
                                gcode.writeExtrusionG1(speed, path.points[point_idx], -retraction_config.distance * theta, path.config->type);
                                continue;
                            }
                            if (point_idx > add_idx_prime)
                            {
                                double theta = 1.0f;
                                if (add_idx_prime != path.points.size() - 2) theta = cur_path_len / travel_dis_retract_prime;
                                gcode.writeExtrusionG1(speed, path.points[point_idx], retraction_config.distance* theta, path.config->type);
                                continue;
                            }
            ...
            {
                gcode.writeTypeComment(path.config->type);
                if (/*path.config->type == PrintFeatureType::PrimeTower &&*/ isNewExtruder)
                {
					int prev_extruder = extruder_nr;
					extruder_nr = extruder_plan.extruder_nr;
			...
			if (path.config->type == PrintFeatureType::SlowFlowTypes)
			{
				for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
				{
				...
				if (path.points.size())
				{
					gcode.writeLine(";TYPE:end Flow");
				}
				continue;
			}
			else if (/*path.retract && */path.config->type == PrintFeatureType::FlowAdanceTypes)
			{
				if (!path.perform_z_hop && final_travel_z != z && extruder_plan_idx == (extruder_plans.size() - 1) && path_idx == (paths.size() - 1))
				{
					...
					//gcode.writeTravel(current_position, extruder->speed_z_hop);
					gcode.writeTravel(current_position, path.config->getSpeed());
				...
				if (path.points.size() > 1)
				{
					...
					coord_t distance = 0;
					int advanceIndex = 0;
					for (int point_idx = path.points.size() - 2; point_idx >= 0; --point_idx)
					{
						Point startPosition = path.points[point_idx];
						Point EndPosition = path.points[point_idx + 1];
						distance += sqrt(pow((EndPosition.X - startPosition.X), 2) + pow((EndPosition.Y - startPosition.Y), 2));

						if (distance > path.length_flow)
						{
							Point advancePosition;
							advancePosition.X = EndPosition.X - path.length_flow * (EndPosition.X - startPosition.X) / distance;
							advancePosition.Y = EndPosition.Y - path.length_flow * (EndPosition.Y - startPosition.Y) / distance;
							path.points.insert(path.points.begin() + point_idx + 1, advancePosition);   //在指定位置，例如在第五个元素前插入一个元素
							advanceIndex = point_idx + 1;
							break;
						}
						else//distance < path.length_flow
						{
							continue;
						}
					}
					...
					for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
					{
						if (point_idx > advanceIndex)//最后一段advance挤出
						{
							gcode.writeLine("M171 T20 ;OPEN_NOZZLE");//G25打开关断指令
							gcode.writeExtrusion(path.points[point_idx], speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset);
						}
						else//空驶
						{
							if (point_idx == 0)
							{
								gcode.writeLine("M170 T20 ;CLOSE_NOZZLE");//G25关断指令
							}
							gcode.writeTravel(path.points[point_idx], path.config->getSpeed());
						}
					}
				...
				{
					Point startPosition = gcode.getPositionXY();
					Point EndPosition = path.points[0];
					Point advancePosition;

					coord_t distance = sqrt(pow((EndPosition.X - startPosition.X), 2) + pow((EndPosition.Y - startPosition.Y), 2));
					if (distance > path.length_flow)
					{
						advancePosition.X = EndPosition.X - path.length_flow * (EndPosition.X - startPosition.X) / distance;
						advancePosition.Y = EndPosition.Y - path.length_flow * (EndPosition.Y - startPosition.Y) / distance;
						gcode.writeLine("M170 T20 ;CLOSE_NOZZLE");//G25关断指令
						gcode.writeTravel(advancePosition, path.config->getSpeed());

						gcode.writeLine("M171 T20 ;OPEN_NOZZLE");//G25打开关断指令
						gcode.writeExtrusion(path.points[0], speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset);
					}
					else
					{
						gcode.writeExtrusion(path.points[0], speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset);
				...
				// for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
				 //{
					 ...
					 //{
					 //	Point currentPosition = gcode.getPositionXY();
					 //	Point EndPosition = path.points[point_idx];
					 //	

					 //	coord_t distance = sqrt(pow((EndPosition.X - currentPosition.X), 2) + pow((EndPosition.Y - currentPosition.Y), 2));
					 //	if (distance > path.length_flow)
					 //	{
	  //                       Point advancePosition;
					 //		advancePosition.X = EndPosition.X - path.length_flow * (EndPosition.X - currentPosition.X) / distance;
					 //		advancePosition.Y = EndPosition.Y - path.length_flow * (EndPosition.Y - currentPosition.Y) / distance;
	  //                       //gcode.writeTravel(advancePosition, speed);
					 //		if (m_meshGroupParams->special_slope_slice_angle > 0)
					 //			gcode.writeTravel_45(advancePosition, extruder->speed_z_hop, m_meshGroupParams, box_x, box_y);
					 //		else
					 //			gcode.writeTravel(advancePosition, extruder->speed_z_hop);
					 //	}
					 //	//gcode.writeExtrusion(path.points[point_idx], speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset);
					 //	if (m_meshGroupParams->special_slope_slice_angle > 0)
					 //		gcode.writeExtrusion45(path.points[point_idx], speed, path.getExtrusionMM3perMM(), path.config->type, m_meshGroupParams, box_x, box_y, update_extrusion_offset);
					 //	else
					 //		gcode.writeExtrusion(path.points[point_idx], speed, path.getExtrusionMM3perMM(), path.config->type, update_extrusion_offset);
	  ...
	  //                   //gcode.writeTravel(path.points[point_idx], speed);
					 //	if (m_meshGroupParams->special_slope_slice_angle > 0)
					 //		gcode.writeTravel_45(path.points[point_idx], extruder->speed_z_hop, m_meshGroupParams, box_x, box_y);
					 //	else
					 //		gcode.writeTravel(path.points[point_idx], extruder->speed_z_hop);
	  //               }
				 //}
				gcode.writeLine(";TYPE:end Advance");
				continue;
			}
            else if (path.config->isTravelPath())
            ...
            { // SPIRALIZE
                ...
                { // handle all consecutive spiralized paths > CHANGES path_idx!
                    ...
                    for (unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                    {
                        ...
                        p0 = p1;
                        if (scene->settings.get<coord_t>("special_slope_slice_angle") == 0)
                        {
...
{
    ExtruderPlan& extruder_plan = extruder_plans[extruder_plan_idx];
    const ExtruderTrain& extruder = scene->extruders[extruder_plan.extruder_nr];
...
void LayerPlan::optimizePaths(const Point& starting_position, Scene* scene)
{
    for (ExtruderPlan& extr_plan : extruder_plans)
    {
        coord_t nozzle_size = scene->extruders[extr_plan.extruder_nr].settings->get<coord_t>("machine_nozzle_size");
        coord_t maximum_deviation = scene->extruders[extr_plan.extruder_nr].settings->get<coord_t>("meshfix_maximum_deviation");

        //Merge paths whose endpoints are very close together into one line.
        MergeInfillLines merger(extr_plan, nozzle_size, maximum_deviation);
