...
#include "cxutil/settings/Settings.h" //For MAX_EXTRUDERS.
#include "cxutil/settings/AngleDegrees.h" //Infill angles.
...
#include "cxutil/util/utilconst.h"
...
{
public:
    cxutil::AABB boundaryBox;       //!< The boundaryBox is an axis-aligned bounardy box which is used to quickly check for possible collision between different parts on different layers. It's an optimalization used during skin calculations.
    ...
    std::vector<Polygons> insets_negtive;
     ...
     */
    cxutil::optional<Polygons> infill_area_own;
...
public:
    ...
    SliceMeshStorage(const SliceMeshStorage& storage);
...
public:
    ...
    Polygons getLayerOutlinessliceBelt(const LayerIndex layer_nr, const bool sliceBelt, const bool include_support, const bool include_prime_tower, const bool external_polys_only = false, const bool for_brim = false) const;
     ...
     */
    cxutil::Polygon getMachineBorder(bool adhesion_offset = false) const;
