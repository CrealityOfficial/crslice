...
void Weaver::weave(MeshGroup* meshgroup, Scene* scene)
{   
    ...
    std::vector<cxutil::Slicer*> slicerList;

    for(Mesh* mesh : meshgroup->meshes)
    {
        constexpr bool variable_layer_heights = false;
        cxutil::Slicer* slicer = new cxutil::Slicer(mesh, connection_height, layer_count,
            variable_layer_heights, &layer_thicknesses, meshgroup);
    ...
    { // find first non-empty layer
        ...
        {
            Polygons parts;
            for (cxutil::Slicer* slicer : slicerList)
    ...
    LOGI("Chainifying layers...\n");
    {
        ...
        scene->communication->sendPolygons(PrintFeatureType::OuterWall, wireFrame.bottom_outline, 1, 1, 1);
        ...
        {
            scene->progress.messageProgress(Progress::Stage::INSET_SKIN, layer_idx+1, layer_count); // abuse the progress system of the normal mode of CuraEngine
            ...
            chainify_polygons(parts1, starting_point_in_layer, chainified, scene);

            scene->communication->sendPolygons(PrintFeatureType::OuterWall, chainified, 1, 1, 1);
    ...
    LOGI("Finding horizontal parts...\n");
    {
        ...
        {
            scene->progress.messageProgress(Progress::Stage::SUPPORT, layer_idx+1, wireFrame.layers.size()); // abuse the progress system of the normal mode of CuraEngine
            ...
            createHorizontalFill(layer, layer_above, scene);
        }
    }
    ...
    { // roofs:
        if (!wireFrame.layers.empty()) //If there are no layers, create no roof.
        {
            ...
            fillRoofs(top_layer.supported, to_be_supported, -1, top_layer.z1, top_layer.roofs, scene);
        }
    }
    ...
    { // bottom:
        if (!wireFrame.layers.empty()) //If there are no layers, create no bottom.
        {
            ...
            fillRoofs(wireFrame.bottom_outline, to_be_supported, -1, wireFrame.layers.front().z0, wireFrame.bottom_infill, scene);
        }
    }
...
void Weaver::createHorizontalFill(WeaveLayer& layer, Polygons& layer_above, Scene* scene)
{
    const coord_t bridgable_dist = scene->current_mesh_group->settings->get<coord_t>("wireframe_height");
...
//     Polygons& polys_below = lower_top_parts;
    ...
    { // roofs
        ...
        fillRoofs(polys_here, to_be_supported, -1, layer.z1, layer.roofs, scene);

    }
   
    { // floors
        Polygons to_be_supported =  polys_above.offset(-bridgable_dist);
        fillFloors(polys_here, to_be_supported, 1, layer.z1, layer.roofs, scene);
    }
    
    {// optimize away doubly printed regions (boundaries of holes in layer etc.)
        for (WeaveRoofPart& inset : layer.roofs.roof_insets)
            connections2moves(inset, scene);
    }
    
}
    

void Weaver::fillRoofs(Polygons& supporting, Polygons& to_be_supported, int direction, int z, WeaveRoof& horizontals, Scene* scene)
{
    ...
    const coord_t roof_inset = scene->current_mesh_group->settings->get<coord_t>("wireframe_roof_inset");
    ...
    for (Polygons inset0 = supporting_outlines; inset0.size() > 0; inset0 = last_inset)
    {
        ...
        connect(last_supported, z, inset1, z, insets.back(), scene);
...
void Weaver::fillFloors(Polygons& supporting, Polygons& to_be_supported, int direction, int z, WeaveRoof& horizontals, Scene* scene)
{
    ...
    for (Polygons outset0 = supporting; outset0.size() > 0; outset0 = outset1)
    {
        ...
        connect(last_supported, z, outset1, z, outsets.back(), scene);
...
void Weaver::connections2moves(WeaveRoofPart& inset, Scene* scene)
{
    const coord_t line_width = scene->current_mesh_group->settings->get<coord_t>("wall_line_width_x");
...
void Weaver::connect(Polygons& parts0, int z0, Polygons& parts1, int z1, WeaveConnection& result, Scene* scene)
{
    ...
    chainify_polygons(parts1, start_close_to, supported, scene);
...
void Weaver::chainify_polygons(Polygons& parts1, Point start_close_to, Polygons& result, Scene* scene)
{
    const Settings& mesh_group_settings = *scene->current_mesh_group->settings;
    const coord_t connection_height = mesh_group_settings.get<coord_t>("wireframe_height");
