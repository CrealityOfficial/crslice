...
#include "cxutil/util/GcodeLayerThreader.h"
#include "infill.h"
...
#include "cxutil/util/orderOptimizer.h"
...
void FffGcodeWriter::reWritePreFixStr(std::string preFix)
{
    gcode.reWritePreFixStr(preFix);
}

void FffGcodeWriter::writeGCode(SliceDataStorage& storage, cxutil::TimeKeeper* time_keeper)
{
    ...
    if (scene->iFunc && scene->iFunc())
    {
        return;
    }
    scene->communication->beginGCode();
    ...
    const bool has_simple_raft = scene->current_mesh_group->settings->get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SIMPLERAFT;
    if (has_raft && storage.raftOutline.size() >0)
    {
        processRaft(storage);
        ...
        for (bool extruder_is_used_in_filler_layers : storage.getExtrudersUsed(-1))
        {
            ...
            {
                process_layer_starting_layer_nr = -Raft::getFillerLayerCount(storage.scene);
                break;
            }
        }
    }
    if (has_simple_raft && storage.raftOutline.size() > 0)
    {
    ...
    const std::function<void (LayerPlan*)>& consume_item =
        ...
        {
            scene->progress.messageProgress(Progress::Stage::EXPORT, std::max(0, gcode_layer->getLayerNr()) + 1, total_layers);
...
void FffGcodeWriter::setConfigFanSpeedLayerTime()
{
    for (const ExtruderTrain& extruder : scene->extruders)
    {
        ...
        fan_speed_layer_time_settings.cool_min_layer_time = extruder.settings->get<Duration>("cool_min_layer_time");
        fan_speed_layer_time_settings.cool_min_layer_time_fan_speed_max = extruder.settings->get<Duration>("cool_min_layer_time_fan_speed_max");
        fan_speed_layer_time_settings.cool_fan_speed_0 = extruder.settings->get<Ratio>("cool_fan_speed_0") * 100.0;
        fan_speed_layer_time_settings.cool_fan_speed_min = extruder.settings->get<Ratio>("cool_fan_speed_min") * 100.0;
        fan_speed_layer_time_settings.cool_fan_speed_max = extruder.settings->get<Ratio>("cool_fan_speed_max") * 100.0;
        fan_speed_layer_time_settings.cool_min_speed = extruder.settings->get<Velocity>("cool_min_speed");
        fan_speed_layer_time_settings.cool_fan_full_layer = extruder.settings->get<LayerIndex>("cool_fan_full_layer");
        if (!extruder.settings->get<bool>("cool_fan_enabled"))
        {
...
void FffGcodeWriter::setConfigRetraction(SliceDataStorage& storage) 
{
    ...
    {
        ExtruderTrain& extruder = scene->extruders[extruder_index];
        RetractionConfig& retraction_config = storage.retraction_config_per_extruder[extruder_index];
        retraction_config.distance = (extruder.settings->get<bool>("retraction_enable")) ? extruder.settings->get<double>("retraction_amount") : 0; //Retraction distance in mm.
        retraction_config.prime_volume = extruder.settings->get<double>("retraction_extra_prime_amount"); //Extra prime volume in mm^3.
        retraction_config.speed = extruder.settings->get<Velocity>("retraction_retract_speed");
        retraction_config.primeSpeed = extruder.settings->get<Velocity>("retraction_prime_speed");
        retraction_config.zHop = extruder.settings->get<coord_t>("retraction_hop");
        retraction_config.retraction_min_travel_distance = extruder.settings->get<coord_t>("retraction_min_travel");
        retraction_config.retraction_extrusion_window = extruder.settings->get<double>("retraction_extrusion_window"); //Window to count retractions in in mm of extruded filament.
        retraction_config.retraction_count_max = extruder.settings->get<size_t>("retraction_count_max");
		retraction_config.travelFlowEnable = extruder.settings->get<bool>("special_travelFlow_enable");
		retraction_config.travelFlow_distance = extruder.settings->get<coord_t>("special_travelFlow_length");
		retraction_config.advanceEnable = extruder.settings->get<bool>("special_advance_enable");
		retraction_config.length_flow_advance = extruder.settings->get<coord_t>("special_advance_length");


        RetractionConfig& switch_retraction_config = storage.extruder_switch_retraction_config_per_extruder[extruder_index];
        switch_retraction_config.distance = extruder.settings->get<double>("switch_extruder_retraction_amount"); //Retraction distance in mm.
        switch_retraction_config.prime_volume = 0.0;
        switch_retraction_config.speed = extruder.settings->get<Velocity>("switch_extruder_retraction_speed");
        switch_retraction_config.primeSpeed = extruder.settings->get<Velocity>("switch_extruder_prime_speed");
        switch_retraction_config.zHop = extruder.settings->get<coord_t>("retraction_hop_after_extruder_switch_height");
...
void FffGcodeWriter::setConfigWipe(SliceDataStorage& storage)
{
    for (size_t extruder_index = 0; extruder_index < scene->extruders.size(); extruder_index++)
    {
        ...
        wipe_config.retraction_enable = extruder.settings->get<bool>("wipe_retraction_enable");
        wipe_config.retraction_config.distance = extruder.settings->get<double>("wipe_retraction_amount");
        wipe_config.retraction_config.speed = extruder.settings->get<Velocity>("wipe_retraction_retract_speed");
        wipe_config.retraction_config.primeSpeed = extruder.settings->get<Velocity>("wipe_retraction_prime_speed");
        wipe_config.retraction_config.prime_volume = extruder.settings->get<double>("wipe_retraction_extra_prime_amount");
        ...
        wipe_config.pause = extruder.settings->get<Duration>("wipe_pause");

        wipe_config.hop_enable = extruder.settings->get<bool>("wipe_hop_enable");
        wipe_config.hop_amount = extruder.settings->get<coord_t>("wipe_hop_amount");
        wipe_config.hop_speed = extruder.settings->get<Velocity>("wipe_hop_speed");

        wipe_config.brush_pos_x = extruder.settings->get<coord_t>("wipe_brush_pos_x");
        wipe_config.repeat_count = extruder.settings->get<size_t>("wipe_repeat_count");
        wipe_config.move_distance = extruder.settings->get<coord_t>("wipe_move_distance");
        wipe_config.move_speed = extruder.settings->get<Velocity>("speed_travel");
        wipe_config.max_extrusion_mm3 = extruder.settings->get<double>("max_extrusion_before_wipe");
        wipe_config.clean_between_layers = extruder.settings->get<bool>("clean_between_layers");
    }
}
...
unsigned int FffGcodeWriter::getStartExtruder(const SliceDataStorage& storage)
{
    ...
    assert(start_extruder_nr < scene->extruders.size() && "start_extruder_nr must be a valid extruder");
...
void FffGcodeWriter::setSupportAngles(SliceDataStorage& storage)
{
    ...
    auto getInterfaceAngles = [&storage, this](const ExtruderTrain& extruder, const std::string& interface_angles_setting, const EFillMethod pattern, const std::string& interface_height_setting)
    {
        ...
        if (angles.empty())
        {
            ...
            else
            {
                ...
                {
                    if (mesh.settings->get<coord_t>(interface_height_setting) >= 2 * scene->current_mesh_group->settings->get<coord_t>("layer_height"))
                    {
...
void FffGcodeWriter::processInitialLayerTemperature(const SliceDataStorage& storage, const size_t start_extruder_nr)
{
    ...
    {
        ExtruderTrain& extruder = scene->extruders[start_extruder_nr];
        constexpr bool wait = true;
        const Temperature print_temp_0 = extruder.settings->get<Temperature>("material_print_temperature_layer_0");
        const Temperature print_temp_here = (print_temp_0 != 0) ? print_temp_0 : extruder.settings->get<Temperature>("material_print_temperature");
    ...
    else if (gcode.getFlavor() != EGCodeFlavor::ULTIGCODE)
    {
        ...
        if (scene->current_mesh_group->settings->get<bool>("material_bed_temp_prepend"))
        {
            if (scene->current_mesh_group->settings->get<bool>("machine_heated_bed"))
            {
                ...
                if (bed_temp != 0)
                {
                    gcode.writeBedTemperatureCommand(bed_temp, scene->vctHotZone,scene->current_mesh_group->settings->get<bool>("material_bed_temp_wait"));
                }
            }
        }

        if (scene->current_mesh_group->settings->get<std::string>("machine_name") == "Creality G12")
        {
            Temperature tempt = scene->current_mesh_group->settings->get<Temperature>("material_heater_band");
            gcode.writeG12TopHeater(tempt);
        }
        ...
        if (scene->current_mesh_group->settings->get<bool>("material_print_temp_prepend"))
        {
            for (unsigned extruder_nr = 0; extruder_nr < num_extruders; extruder_nr++)
            {
                if (extruder_is_used[extruder_nr])
                {
                    ...
                    {
                        const Temperature print_temp_0 = extruder.settings->get<Temperature>("material_print_temperature_layer_0");
                        extruder_temp = (print_temp_0 != 0) ? print_temp_0 : extruder.settings->get<Temperature>("material_print_temperature");
                    }
                    else
                    {
                        extruder_temp = extruder.settings->get<Temperature>("material_standby_temperature");//´ý»úÎÂ¶È
                    }
            ...
            if (scene->current_mesh_group->settings->get<bool>("material_print_temp_wait"))
            {
                for (unsigned extruder_nr = 0; extruder_nr < num_extruders; extruder_nr++)
                {
                    if (extruder_is_used[extruder_nr])
                    {
                        ...
                        {
                            extruder_temp = extruder.settings->get<Temperature>("material_standby_temperature");
                        }
...
{
    std::vector<bool> extruder_is_used = storage.getExtrudersUsed();
    if (scene->communication->isSequential()) //If we must output the g-code sequentially, we must already place the g-code header here even if we don't know the exact time/material usages yet.
    {
    ...
    scene->communication->sendCurrentPosition(gcode.getPositionXY());
...
void FffGcodeWriter::processNextMeshGroupCode(const SliceDataStorage& storage)
{
    ...
    scene->communication->sendCurrentPosition(gcode.getPositionXY());
    gcode.writeTravel(gcode.getPositionXY(), scene->extruders[gcode.getExtruderNr()].settings->get<Velocity>("speed_travel"));
    Point start_pos(storage.model_min.x, storage.model_min.y);
    gcode.writeTravel(start_pos, scene->extruders[gcode.getExtruderNr()].settings->get<Velocity>("speed_travel"));
    ...
    if (mesh_group_settings.get<bool>("machine_heated_bed") && mesh_group_settings.get<Temperature>("material_bed_temperature_layer_0") != 0)
    {
        ...
        gcode.writeBedTemperatureCommand(mesh_group_settings.get<Temperature>("material_bed_temperature_layer_0"),scene->vctHotZone, wait);
    }
...
{
    Settings& mesh_group_settings = *scene->current_mesh_group->settings;
    size_t extruder_nr = mesh_group_settings.get<ExtruderTrain&>("adhesion_extruder_nr").extruder_nr;
    const ExtruderTrain& extruder = scene->extruders[extruder_nr];

    coord_t z = 0;
    const LayerIndex initial_raft_layer_nr = -Raft::getTotalExtraLayers(storage.scene);
    ...
    cxutil::optional<Point> last_planned_position = cxutil::optional<Point>();

    unsigned int current_extruder_nr = extruder_nr;

    { // raft base layer
        LayerIndex layer_nr = initial_raft_layer_nr;
        const coord_t layer_height = extruder.settings->get<coord_t>("raft_base_thickness");
        z += layer_height;
        const coord_t comb_offset = extruder.settings->get<coord_t>("raft_base_line_spacing");
        ...
        {
            double regular_fan_speed = extruder.settings->get<Ratio>("raft_base_fan_speed") * 100.0;
        ...
        LayerPlan* gcode_layer = new LayerPlan(storage, layer_nr, z, layer_height, extruder_nr, fan_speed_layer_time_settings_per_extruder_raft_base, comb_offset, extruder.settings->get<bool>("raft_base_line_width"), extruder.settings->get<coord_t>("travel_avoid_distance"));
        gcode_layer->setIsInside(true);

        gcode_layer->setExtruder(extruder_nr);

        scene->communication->sendLayerComplete(layer_nr, z, layer_height);
        ...
        Infill infill_comp(
            EFillMethod::LINES, zig_zaggify_infill, connect_polygons, wall, offset_from_poly_outline, gcode_layer->configs_storage.raft_base_config.getLineWidth(), extruder.settings->get<coord_t>("raft_base_line_spacing"),
    ...
    { // raft interface layer
        ...
        const coord_t layer_height = extruder.settings->get<coord_t>("raft_interface_thickness");
        z += layer_height;
        const coord_t comb_offset = extruder.settings->get<coord_t>("raft_interface_line_spacing");
        ...
        {
            double regular_fan_speed = extruder.settings->get<Ratio>("raft_interface_fan_speed") * 100.0;
            fan_speed_layer_time_settings.cool_fan_speed_min = regular_fan_speed;
            fan_speed_layer_time_settings.cool_fan_speed_0 = regular_fan_speed; // ignore initial layer fan speed stuff
        }

        LayerPlan* gcode_layer = new LayerPlan(storage, layer_nr, z, layer_height, current_extruder_nr, fan_speed_layer_time_settings_per_extruder_raft_interface, comb_offset, extruder.settings->get<coord_t>("raft_interface_line_width"), extruder.settings->get<coord_t>("travel_avoid_distance"));
        gcode_layer->setIsInside(true);
        ...
        AngleDegrees fill_angle = extruder.settings->get<size_t>("raft_surface_layers") > 0 ? 45 : 90;
        ...
        Infill infill_comp(
            EFillMethod::ZIG_ZAG, zig_zaggify_infill, connect_polygons, raft_outline_path, offset_from_poly_outline, infill_outline_width, extruder.settings->get<coord_t>("raft_interface_line_spacing"),
            fill_overlap, infill_multiplier, fill_angle, z, extra_infill_shift,
            wall_line_count, infill_origin, perimeter_gaps, connected_zigzags, use_endpieces, skip_some_zags, zag_skip_count, pocket_size
            );
        infill_comp.generate(raft_polygons, raftLines);
    ...
    coord_t layer_height = extruder.settings->get<coord_t>("raft_surface_thickness");

    for (LayerIndex raft_surface_layer = 1; static_cast<size_t>(raft_surface_layer) <= extruder.settings->get<size_t>("raft_surface_layers"); raft_surface_layer++)
    { // raft surface layers
        const LayerIndex layer_nr = initial_raft_layer_nr + 2 + raft_surface_layer - 1; // 2: 1 base layer, 1 interface layer
        z += layer_height;
        const coord_t comb_offset = extruder.settings->get<coord_t>("raft_surface_line_spacing");
        ...
        {
            double regular_fan_speed = extruder.settings->get<Ratio>("raft_surface_fan_speed") * 100.0;
            fan_speed_layer_time_settings.cool_fan_speed_min = regular_fan_speed;
            fan_speed_layer_time_settings.cool_fan_speed_0 = regular_fan_speed; // ignore initial layer fan speed stuff
        }

        LayerPlan* gcode_layer = new LayerPlan(storage, layer_nr, z, layer_height, extruder_nr, fan_speed_layer_time_settings_per_extruder_raft_surface, comb_offset, extruder.settings->get<coord_t>("raft_surface_line_width"), extruder.settings->get<coord_t>("travel_avoid_distance"));
        gcode_layer->setIsInside(true);
        ...
        const coord_t maximum_resolution = extruder.settings->get<coord_t>("meshfix_maximum_resolution");
        const coord_t maximum_deviation = extruder.settings->get<coord_t>("meshfix_maximum_deviation");
        ...
        Infill infill_comp(
            EFillMethod::ZIG_ZAG, zig_zaggify_infill, connect_polygons, raft_outline_path, offset_from_poly_outline, infill_outline_width, extruder.settings->get<coord_t>("raft_surface_line_spacing"),
            fill_overlap, infill_multiplier, fill_angle, z, extra_infill_shift,
            wall_line_count, infill_origin, perimeter_gaps, connected_zigzags, use_endpieces, skip_some_zags, zag_skip_count, pocket_size
            );
...
{
    //GCodeData& tdata = output->data;
    //SliceDataStorage& storage = output->storage;
    //LPBuffer* layer_plan_buffer = buffer;//m_pathCombine->lpbuffer(index);
    //MeshGroupParam* mgParam = storage.scene->currentModelGroup()->param();
    Settings& mesh_group_settings = *scene->current_mesh_group->settings;
    size_t extruder_nr = mesh_group_settings.get<ExtruderTrain&>("adhesion_extruder_nr").extruder_nr;
    const ExtruderTrain& extruder = scene->extruders[extruder_nr];

   // ExtruderParam* eparam = storage.scene->extruders().at(extruder_nr)->param();
    int raft_init_layer_num = extruder.settings->get<size_t>("raft_init_layer_num");
    int raft_top_layer_num = extruder.settings->get<size_t>("raft_top_layer_num");
    double raft_init_flow_ratio = extruder.settings->get<size_t>("raft_init_flow_ratio");
    coord_t z = 0;
    const LayerIndex initial_raft_layer_nr = -Raft::getTotalSimpleExtraLayers(storage.scene)-1;//interface layer
    ...
    { // simpleraft init layer
        LayerIndex layer_nr = initial_raft_layer_nr + raft_init_layer;
        const coord_t layer_height = extruder.settings->get<coord_t>("raft_base_thickness");
        ...
        for (FanSpeedLayerTimeSettings& fan_speed_layer_time_settings : fan_speed_layer_time_settings_per_extruder_raft_base)
        {
            double regular_fan_speed = extruder.settings->get<double>("raft_base_fan_speed") * 100.0;
            fan_speed_layer_time_settings.cool_fan_speed_min = regular_fan_speed;
            fan_speed_layer_time_settings.cool_fan_speed_0 = regular_fan_speed; // ignore initial layer fan speed stuff
        }
        ...
        const coord_t raft_base_space = extruder.settings->get<coord_t>("raft_base_line_spacing");
        const coord_t line_width = gcode_layer->configs_storage.raft_base_config.getLineWidth();
        Infill infill_comp(
            EFillMethod::LINES, zig_zaggify_infill, connect_polygons, wall, offset_from_poly_outline, line_width, raft_base_space,
            fill_overlap, infill_multiplier, fill_angle, z, extra_infill_shift,
            wall_line_count, infill_origin, perimeter_gaps, connected_zigzags, use_endpieces, skip_some_zags, zag_skip_count, pocket_size
        );
        infill_comp.generate(raft_polygons, raftLines);
        if (raftLines.empty()) continue;
        gcode_layer->addLinesByOptimizer(raftLines, gcode_layer->configs_storage.raft_base_config, SpaceFillType::Lines, false, 0, raft_init_flow_ratio / 100.0f);
        // When we use raft, we need to make sure that all used extruders for this print will get primed on the first raft layer,
        // and then switch back to the original extruder.
        std::vector<size_t> extruder_order = getUsedExtrudersOnLayerExcludingStartingExtruder(storage, extruder_nr, layer_nr);
        for (const size_t to_be_primed_extruder_nr : extruder_order)
    ...
    { // raft interface layer
        const LayerIndex layer_nr = initial_raft_layer_nr + raft_init_layer_num;
        const coord_t layer_height = extruder.settings->get<coord_t>("raft_interface_thickness");
        z += layer_height;
        const coord_t comb_offset = extruder.settings->get<coord_t>("raft_interface_line_spacing");
        ...
        for (FanSpeedLayerTimeSettings& fan_speed_layer_time_settings : fan_speed_layer_time_settings_per_extruder_raft_interface)
        {
            double regular_fan_speed = extruder.settings->get<double>("raft_interface_fan_speed") * 100.0;
            fan_speed_layer_time_settings.cool_fan_speed_min = regular_fan_speed;
            fan_speed_layer_time_settings.cool_fan_speed_0 = regular_fan_speed; // ignore initial layer fan speed stuff
        }
        ...
        Polygons raftLines;
        int offset_from_poly_outline = 0;
        AngleDegrees fill_angle = extruder.settings->get<AngleDegrees>("raft_surface_layers") > 0 ? 45 : 90;
        constexpr bool zig_zaggify_infill = true;
        constexpr bool connect_polygons = true; // why not?
        ...
        Infill infill_comp(
            EFillMethod::LINES, zig_zaggify_infill, connect_polygons, raft_outline_path, offset_from_poly_outline, infill_outline_width, extruder.settings->get<coord_t>("raft_interface_line_spacing"),
            fill_overlap, infill_multiplier, fill_angle, z, extra_infill_shift,
            wall_line_count, infill_origin, perimeter_gaps, connected_zigzags, use_endpieces, skip_some_zags, zag_skip_count, pocket_size
        );
        infill_comp.generate(raft_polygons, raftLines);
    ...
    { // raft surface layers
        const LayerIndex layer_nr = initial_raft_layer_nr + raft_init_layer_num + raft_surface_layer; //1 interface layer
        z += layer_height;
        const coord_t comb_offset = extruder.settings->get<coord_t>("raft_surface_line_spacing");
        ...
        for (FanSpeedLayerTimeSettings& fan_speed_layer_time_settings : fan_speed_layer_time_settings_per_extruder_raft_surface)
        {
            double regular_fan_speed = extruder.settings->get<double>("raft_surface_fan_speed") * 100.0;
            fan_speed_layer_time_settings.cool_fan_speed_min = regular_fan_speed;
            fan_speed_layer_time_settings.cool_fan_speed_0 = regular_fan_speed; // ignore initial layer fan speed stuff
        }
        ...
        Infill infill_comp(
            EFillMethod::LINES, zig_zaggify_infill, connect_polygons, raft_outline_path, offset_from_poly_outline, infill_outline_width, extruder.settings->get<coord_t>("raft_surface_line_spacing"),
            fill_overlap, infill_multiplier, fill_angle, z, extra_infill_shift,
            wall_line_count, infill_origin, perimeter_gaps, connected_zigzags, use_endpieces, skip_some_zags, zag_skip_count, pocket_size
        );
        infill_comp.generate(raft_polygons, raft_lines);
...
{
   // LOGD("GcodeWriter processing layer %i of %i\n", layer_nr, total_layers);
    const Settings& mesh_group_settings = *scene->current_mesh_group->settings;
    ...
    {
#ifdef DEBUG
        assert((mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT || mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SIMPLERAFT) && "negative layer_number means post-raft, pre-model layer!");
#endif // DEBUG
        const int filler_layer_count = Raft::getFillerLayerCount(scene);
        layer_thickness = Raft::getFillerLayerHeight(scene);
        z = Raft::getTotalThickness(scene) + (filler_layer_count + layer_nr + 1) * layer_thickness;
    }
    else
    {
...
#pragma omp critical
    scene->communication->sendLayerComplete(layer_nr, z, layer_thickness);
    ...
    for (const SliceMeshStorage& mesh : storage.meshes)
    {
        ...
        if (layer_nr == 0)
        {
            const ExtruderTrain& extruder = mesh.settings->get<ExtruderTrain&>((mesh.settings->get<size_t>("wall_line_count") > 1) ? "wall_0_extruder_nr" : "wall_x_extruder_nr");
            max_inner_wall_width *= extruder.settings->get<Ratio>("initial_layer_line_width_factor");
        }
    }
    ...
    {
        gcode_layer->optimizePaths(gcode.getPositionXY(), scene);
    }
...
void FffGcodeWriter::processSkirtBrim(const SliceDataStorage& storage, LayerPlan* gcode_layer, unsigned int extruder_nr) const
{
    ...
    if (extruder.settings->get<bool>("prime_blob_enable"))
    {
        const bool prime_pos_is_abs = extruder.settings->get<bool>("extruder_prime_pos_abs");
        const Point prime_pos(extruder.settings->get<coord_t>("extruder_prime_pos_x"), extruder.settings->get<coord_t>("extruder_prime_pos_y"));
    ...
    if (extruder.settings->get<bool>("brim_outside_only"))
    {
...
void FffGcodeWriter::processOozeShield(const SliceDataStorage& storage, LayerPlan* gcode_layer) const
{
    ...
    if (layer_nr == 0 && scene->current_mesh_group->settings->get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::BRIM)
    {
...
void FffGcodeWriter::calculateExtruderOrderPerLayer(const SliceDataStorage& storage)
{
    ...
    for (LayerIndex layer_nr = -Raft::getTotalExtraLayers(scene); layer_nr < static_cast<LayerIndex>(storage.print_layer_count); layer_nr++)
    {
...
{
    const Settings& mesh_group_settings = *scene->current_mesh_group->settings;
    size_t extruder_count = scene->extruders.size();
    ...
    if (((mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT || mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SIMPLERAFT)&& layer_nr == -static_cast<LayerIndex>(Raft::getTotalExtraLayers(scene)))
...
std::vector<size_t> FffGcodeWriter::calculateMeshOrder(const SliceDataStorage& storage, const size_t extruder_nr) const
{
    ...
    const ExtruderTrain& extruder = scene->extruders[extruder_nr];
    const Point layer_start_position(extruder.settings->get<coord_t>("layer_start_x"), extruder.settings->get<coord_t>("layer_start_y"));
...
void FffGcodeWriter::addMeshLayerToGCode_meshSurfaceMode(const SliceDataStorage& storage, const SliceMeshStorage& mesh, const PathConfigStorage::MeshPathConfigs& mesh_config, LayerPlan* gcode_layer) const
{
    ...
    ZSeamConfig z_seam_config(mesh.settings->get<EZSeamType>("z_seam_type"), mesh.getZSeamHint(), mesh.settings->get<EZSeamCornerPrefType>("z_seam_corner")
        , storage.scene->settings.get<coord_t>("z_seam_min_angle_diff") / 1000.f, storage.scene->settings.get<coord_t>("z_seam_max_angle") / 1000.f);
    const bool spiralize = scene->current_mesh_group->settings->get<bool>("magic_spiralize");
...
bool FffGcodeWriter::processMultiLayerInfill(const SliceDataStorage& storage, LayerPlan* gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    ...
    for(unsigned int combine_idx = 1; combine_idx < part.infill_area_per_combine_per_density[0].size(); combine_idx++)
    {
        ...
        if (!infill_lines.empty() || !infill_polygons.empty())
        {
            ...
            cxutil::optional<Point> near_start_location;
...
//     ^   highest density line dist
    ...
    if (infill_lines.size() > 0 || infill_polygons.size() > 0)
    {
        ...
        //preview
        if (scene->debugCallback)
        {
            scene->debugCallback->onZPaths(&infill_lines.paths, gcode_layer->z, 3);
            scene->debugCallback->onZPaths(&infill_polygons.paths, gcode_layer->z, 3);
        }
    }
    return added_something;
}
...
bool FffGcodeWriter::processInsets(const SliceDataStorage& storage, LayerPlan* gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    ...
    if (mesh.settings->get<size_t>("wall_line_count") > 0)
    {
        ...
        if (!spiralize && gcode_layer->getLayerNr() > 0)
        {
            ...
            else
            {
                ...
                if (mesh.settings->get<bool>("set_wall_overhang_grading"))
                {
                    ...
                    for (int i = 1; i < 5; i++)
                    {
                        const coord_t _overhang_width = layer_height * std::tan(overhang_angle_level[i] / (180 / M_PI));
        ...
        {
            float z_seam_min_angle_diff = storage.scene->settings.get<coord_t>("z_seam_min_angle_diff") / 1000.f;
            float z_seam_max_angle = storage.scene->settings.get<coord_t>("z_seam_max_angle") / 1000.f;
            InsetOrderOptimizer ioo(*this, storage, gcode_layer, mesh, extruder_nr, mesh_config, part, gcode_layer->getLayerNr(), z_seam_min_angle_diff, z_seam_max_angle);
...
cxutil::optional<Point> FffGcodeWriter::getSeamAvoidingLocation(const Polygons& filling_part, int filling_angle, Point last_position) const
{
    if (filling_part.empty())
    {
        return cxutil::optional<Point>();
    }
    ...
    {
        bool bs_arg = true;
        return cxutil::optional<Point>(bs_arg, pa.p());
    }
    else
    {
        bool bs_arg = true;
        return cxutil::optional<Point>(bs_arg, pb.p());
    }
}
...
bool FffGcodeWriter::processSkinAndPerimeterGaps(const SliceDataStorage& storage, LayerPlan* gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    ...
    const bool fill_perimeter_gaps = mesh.settings->get<FillPerimeterGapMode>("fill_perimeter_gaps") != FillPerimeterGapMode::NOWHERE
                            && !scene->current_mesh_group->settings->get<bool>("magic_spiralize")
...
bool FffGcodeWriter::processSupportInfill(const SliceDataStorage& storage, LayerPlan* gcode_layer) const
{
    ...
    const ExtruderTrain& infill_extruder = scene->extruders[extruder_nr];
    ...
    const bool support_tree_enable = infill_extruder.settings->get<bool>("support_tree_enable");
    ...
    for (int part_idx : island_order_optimizer.polyOrder)
    {
        ...
        if (!part.insets.empty())
        {
            ...
            if (all_insets.size() > 0&& support_tree_enable)
            {
        ...
        ClipperLib::Paths supportViewPaths;
        for (unsigned int combine_idx = 0; combine_idx < part.infill_area_per_combine_per_density[0].size(); ++combine_idx)
        {
            ...
            for (unsigned int density_idx = part.infill_area_per_combine_per_density.size() - 1; (int)density_idx >= 0; density_idx--)
            {
                ...
                if (need_adhesion)
                {
                    if (mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT || mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SIMPLERAFT)
                    {
                        ...
                        support_line_distance_here = 2 * support_line_width;
                    }                      
                    else
                    {
                        support_pattern = EFillMethod::CONCENTRIC;
                        support_line_distance_here = support_line_width;
                    }
                }
            ...
            if (scene->debugCallback)
            {
                for (ClipperLib::Path& apath : support_polygons.paths)
                {
                    supportViewPaths.push_back(apath);
                }
                for (ClipperLib::Path& apath : support_lines.paths)
                {
                    supportViewPaths.push_back(apath);
                }
            }
        ...
        if (scene->debugCallback)
            scene->debugCallback->onZPaths(&supportViewPaths, gcode_layer->z);
    }
...
bool FffGcodeWriter::addSupportRoofsToGCode(const SliceDataStorage& storage, LayerPlan* gcode_layer) const
{
    ...
    const size_t roof_extruder_nr = scene->current_mesh_group->settings->get<ExtruderTrain&>("support_roof_extruder_nr").extruder_nr;
    const ExtruderTrain& roof_extruder = scene->extruders[roof_extruder_nr];
    ...
    if (scene->debugCallback)
        scene->debugCallback->onZPaths(&roof_lines.paths, gcode_layer->z,1);
    return true;
}
...
bool FffGcodeWriter::addSupportBottomsToGCode(const SliceDataStorage& storage, LayerPlan* gcode_layer) const
{
    ...
    const size_t bottom_extruder_nr = scene->current_mesh_group->settings->get<ExtruderTrain&>("support_bottom_extruder_nr").extruder_nr;
    const ExtruderTrain& bottom_extruder = scene->extruders[bottom_extruder_nr];
    ...
    if (scene->debugCallback)
        scene->debugCallback->onZPaths(&bottom_lines.paths, gcode_layer->z, 2);
    return true;
}
...
void FffGcodeWriter::setExtruder_addPrime(const SliceDataStorage& storage, LayerPlan* gcode_layer, const size_t extruder_nr) const
{
    ...
    if (previous_extruder == extruder_nr && !(extruder_nr == outermost_prime_tower_extruder
            && gcode_layer->getLayerNr() >= -static_cast<LayerIndex>(Raft::getFillerLayerCount(scene)))) //No unnecessary switches, unless switching to extruder for the outer shell of the prime tower.
    {
        return;
    }
    ...
    if (extruder_changed)
    {
        if (extruder_prime_layer_nr[extruder_nr] == gcode_layer->getLayerNr())
        {
            ...
            if (extruder.settings->get<bool>("prime_blob_enable")) // Don't travel to the prime-blob position if not enabled though.
            {
                bool prime_pos_is_abs = extruder.settings->get<bool>("extruder_prime_pos_abs");
                Point prime_pos = Point(extruder.settings->get<coord_t>("extruder_prime_pos_x"), extruder.settings->get<coord_t>("extruder_prime_pos_y"));
    ...
    // first layer again.
    if (((extruder_changed && gcode_layer->getLayerNr() >= 0) || extruder_nr == outermost_prime_tower_extruder) && gcode_layer->getLayerNr() >= -static_cast<LayerIndex>(Raft::getFillerLayerCount(scene))) //Always print a prime tower with outermost extruder.
    {
...
{
    if (!scene->current_mesh_group->settings->get<bool>("prime_tower_enable"))
    {
        return;
    }
...
void FffGcodeWriter::finalize()
{
    ...
    {
        gcode.writeBedTemperatureCommand(0,scene->vctHotZone); //Cool down the bed (M140).
    ...
    if (!scene->communication->isSequential())
    {
        scene->communication->sendGCodePrefix(prefix);
    }
    else
    {
        LOGI("Gcode header after slicing:\n");
        LOGI("%s", prefix.c_str());
        LOGI("End of gcode header.\n");

        gcode.reWritePreFixStr(prefix);
        if (scene->callback)
        {
            scene->callback->onLayerCount(gcode.getTotalGcodeLayers());
        }
    ...
    for (size_t e = 0; e < scene->extruders.size(); e++)
    {
