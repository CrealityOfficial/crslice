...
#include "cxutil/settings/AngleRadians.h" //For the infill support angle.
...
SkinInfillAreaComputation::SkinInfillAreaComputation(const LayerIndex& layer_nr, SliceMeshStorage& mesh, bool process_infill,bool hasInset)
: layer_nr(layer_nr)
, mesh(mesh)
...
{
    Polygons original_outline = part.insets.back();
    const size_t hole_wall_line_count = mesh.settings->get<size_t>("hole_wall_line_count");
    if (hole_wall_line_count < mesh.settings->get<size_t>("wall_line_count") && !part.insets_negtive[hole_wall_line_count].empty())
    {
        original_outline.add(part.insets_negtive[hole_wall_line_count - 1]);
    }
    original_outline = original_outline.offset(-innermost_wall_line_width / 2);
...
void SkinInfillAreaComputation::generateSkinInsetsAndInnerSkinInfill(SliceLayerPart* part)
{
    ...
    {
        if (m_has_inset) generateSkinInsets(skin_part);
...
void SkinInfillAreaComputation::generateInfill(SliceLayerPart& part, const Polygons& skin)
{
    ...
    if (hole_wall_line_count < mesh.settings->get<size_t>("wall_line_count") && !part.insets_negtive[hole_wall_line_count].empty())
    {
        infill.add(part.insets_negtive[hole_wall_line_count - 1].offset(-offset_from_inner_wall));
    }
