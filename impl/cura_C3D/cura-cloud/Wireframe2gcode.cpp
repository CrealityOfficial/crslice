...
{
    Settings& scene_settings = scene->settings;
    ...
    {
        scene->progress.messageProgress(Progress::Stage::EXPORT, layer_nr+1, total_layers); // abuse the progress system of the normal mode of CuraEngine
...
Wireframe2gcode::Wireframe2gcode(Weaver& weaver, GCodeExport& gcode, Scene* _scene)
: gcode(gcode)
, wireFrame(weaver.wireFrame)
, scene(_scene)
{
    const Settings& scene_settings = scene->settings;
...
void Wireframe2gcode::processStartingCode()
{
    ...
    if (gcode.getFlavor() != EGCodeFlavor::ULTIGCODE && gcode.getFlavor() != EGCodeFlavor::GRIFFIN)
    {
        if (scene_settings.get<bool>("material_bed_temp_prepend"))
        {
            ...
            {
                gcode.writeBedTemperatureCommand(scene_settings.get<Temperature>("material_bed_temperature"),scene->vctHotZone, scene_settings.get<bool>("material_bed_temp_wait"));
            }
        }
...
{
    gcode.finalize(scene->settings.get<std::string>("machine_end_gcode").c_str());
