...
#include "cxutil/infill/DensityProvider.h" // for destructor
...
void SupportStorage::write(std::fstream& out)
{
    auto fpolygons = [&out](const Polygons& paths)
    {
        for (size_t p = 0; p < paths.size(); ++p)
        {
            const ClipperLib::Path& path = *paths[p];
    ...
    for (size_t i = 0; i < supportLayers.size(); ++i)
    {
        const SupportLayer& slayer = supportLayers.at(i);
        out << "layer " << i << std::endl;
        out << "anti_overhang " << std::endl;
        fpolygons(slayer.anti_overhang);
		out << "support_bottom " << std::endl;
		fpolygons(slayer.support_bottom);
		out << "support_mesh " << std::endl;
		fpolygons(slayer.support_mesh);
		out << "support_mesh_drop_down " << std::endl;
		fpolygons(slayer.support_mesh_drop_down);
		out << "support_roof " << std::endl;
		fpolygons(slayer.support_roof);
		for (size_t j = 0; j < slayer.support_infill_parts.size(); ++j)
		{
			const SupportInfillPart& sinfillPart = slayer.support_infill_parts.at(j);
			out << "support_infill_parts " << j << std::endl;
			out << "outline" << std::endl;
			fpolygons(sinfillPart.outline);
			for (size_t k = 0; k < sinfillPart.insets.size(); ++k)
			{
				out << "support_infill_parts insets" << k << std::endl;
				fpolygons(sinfillPart.insets.at(k));
			}
		}
...
SliceMeshStorage::SliceMeshStorage(const SliceMeshStorage& storage)
...
void SliceMeshStorage::write(std::fstream& out)
{
    auto fpolygons = [&out](const Polygons& paths)
    {
        for (size_t p = 0; p < paths.size(); ++p)
    ...
    {
        const SliceLayer& slayer = layers.at(i);
        const std::vector<SliceLayerPart>& layerParts = slayer.parts;
        out << "layer " << i << std::endl;
        for (size_t j = 0; j < layerParts.size(); ++j)
        {
            const SliceLayerPart& layerPart = layerParts.at(j);
            out << "part " << j << std::endl;
            out << "outline " << std::endl;
            fpolygons(layerPart.outline);
            out << "print " << std::endl;
            fpolygons(layerPart.print_outline);
            for (size_t k = 0; k < layerPart.insets.size(); ++k)
            {
                out << "inset " << k << std::endl;
                fpolygons(layerPart.insets.at(k));
            }
			out << "aabb " << std::endl;
			out << layerPart.boundaryBox.min << " " << layerPart.boundaryBox.max << std::endl;
			out << "infill_area" << std::endl;
			fpolygons(layerPart.infill_area);
			for (size_t k = 0; k < layerPart.skin_parts.size(); ++k)
			{
				const SkinPart& skinPart = layerPart.skin_parts.at(k);
				out << "skin outline" << std::endl;
				fpolygons(skinPart.outline);
				out << "skin innerfill" << std::endl;
				fpolygons(skinPart.inner_infill);
				for (size_t h = 0; h < skinPart.insets.size(); ++h)
				{
					out << "skin inset " << h <<  std::endl;
					fpolygons(skinPart.insets.at(h));
				}
			}
        }

        out << "overhang areas" << std::endl;
        fpolygons(overhang_areas.at(i));
        out << "full overhang areas" << std::endl;
        fpolygons(full_overhang_areas.at(i));
        out << std::endl << std::endl << std::endl;
    }
}
...
std::vector<RetractionConfig> SliceDataStorage::initializeRetractionConfigs()
{
    ...
    ret.resize(scene->extruders.size()); // initializes with constructor RetractionConfig()
    return ret;
}
...
std::vector<WipeScriptConfig> SliceDataStorage::initializeWipeConfigs()
{
    ...
    ret.resize(scene->extruders.size());
    return ret;
}

SliceDataStorage::SliceDataStorage(Scene* _scene)
: print_layer_count(0)
, scene(_scene)
...
Polygons SliceDataStorage::getLayerOutlinessliceBelt(const LayerIndex layer_nr, const bool sliceBelt, const bool include_support, const bool include_prime_tower, const bool external_polys_only, const bool for_brim) const
{
    if (layer_nr < 0 && layer_nr < -static_cast<LayerIndex>(Raft::getFillerLayerCount(scene)))
...
Polygons SliceDataStorage::getLayerOutlines(const LayerIndex layer_nr, const bool include_support, const bool include_prime_tower, const bool external_polys_only, const bool for_brim) const
{
    ...
    else
    {
        ...
        if (include_support && support.supportLayers.size() > 0)
        {
            const SupportLayer& support_layer = support.supportLayers[std::max(LayerIndex(0), layer_nr)];
            if (support.generated)
            {
...
{
    std::vector<bool> ret;
    ret.resize(scene->extruders.size(), false);
...
std::vector<bool> SliceDataStorage::getExtrudersUsed(LayerIndex layer_nr) const
{
    ...
    if (layer_nr < 0)
    {
        ...
        if (layer_nr < -static_cast<LayerIndex>(Raft::getFillerLayerCount(scene)))
        {
    ...
    else if (layer_nr > 0 || mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT || mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::SIMPLERAFT)
...
{
    if (extruder_nr >= scene->extruders.size())
    {
        return false;
    }

    const ExtruderTrain& extruder = scene->extruders[extruder_nr];
    return extruder.settings->get<bool>("prime_blob_enable");
}

cxutil::Polygon SliceDataStorage::getMachineBorder(bool adhesion_offset) const
{
    const Settings& mesh_group_settings = *scene->current_mesh_group->settings;

    cxutil::Polygon border{};
    ...
    for (size_t extruder_nr = 0; extruder_nr < scene->extruders.size(); extruder_nr++)
    {
        ...
        const ExtruderTrain& other_extruder = scene->extruders[extruder_nr];
